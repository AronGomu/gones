...
| `.agents/skills/`                        | project skills: `impeccable` (frontend design), `ship` (engineering pipeline), `start-gones-server`                                                                                                                   |
...
- **Every page that reads server data joins the cache contract.** Load once on page load, cache for
  ...
  `/about` is the display-only exception: its upcoming MTGones events use the public catalog cache
  without rendering the last-synced label or Synchronize button (ADR 0039).
- **Every routed page carries a back button at the top and at the bottom** (`gones-back-button`,
  `position="top"` and `position="bottom"`), except the pages that _start_ a breadcrumb (`/` and
  `/admin`, which render none), auth exception pages (top only), and `/about` (bottom only because its section
  navigation occupies the main header). Enforced by
