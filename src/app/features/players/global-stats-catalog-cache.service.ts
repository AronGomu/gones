...
import { Client, GlobalPlayerStatisticsRow } from '../../api/generated/gones-api';
...
type StoredEntry = CatalogEntry<GlobalPlayerStatisticsRow[]>;

@Injectable({ providedIn: 'root' })
export class GlobalStatsCatalogCacheService {
  private readonly client = inject(Client);

  async load(options: { force?: boolean } = {}): Promise<CatalogResult<GlobalPlayerStatisticsRow[]>> {
    const cached = readCatalogEntry<GlobalPlayerStatisticsRow[]>(GLOBAL_STATS_CACHE_KEY);
    ...
    try {
      const response = await firstValueFrom(this.client.getGlobalPlayerStatisticsCatalog());
