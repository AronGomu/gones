...
@Component({
  ...
  template: `
    <section class="tournament-management-page stack" data-cy="deleted-events" aria-labelledby="deleted-events-title">
      ...
      @else if (!items().length) { <div class="panel stack" data-cy="deleted-events-empty"><h2 data-cy="deleted-events-empty-title">{{ i18n.t('tournamentManage.deletedEmpty') }}</h2></div> }
      @else {
          ...
          @for (event of items(); track event.id) {
            <mat-card class="panel tournament-management-row" role="listitem" [attr.data-cy]="'deleted-event-row-' + event.id"><mat-card-content [attr.data-cy]="'deleted-event-content-' + event.id">
              <div class="tournament-management-row-grid" [attr.data-cy]="'deleted-event-grid-' + event.id">
                ...
                <div class="admin-actions" [attr.data-cy]="'deleted-event-actions-' + event.id"><button mat-flat-button type="button" data-cy="event-restore" [disabled]="!!pendingId()" (click)="restore(event)">{{ pendingId() === event.id ? i18n.t('tournamentManage.restoring') : i18n.t('admin.restore') }}</button></div>
              </div>
