...
@Component({
  ...
  template: `
    ...
    <mat-dialog-content class="stack">
      <p>{{ data.scope }}</p>
      <label for="participant-block-reason">{{ data.reasonLabel }}</label>
      <input id="participant-block-reason" data-cy="block-reason" [(ngModel)]="reason" maxlength="500" />
      <label for="participant-block-expiry">{{ data.expiryLabel }}</label>
      <input id="participant-block-expiry" data-cy="block-expiry" type="datetime-local" [(ngModel)]="expiresLocal" />
      <p class="muted">{{ data.expiryHelp }}</p>
    </mat-dialog-content>
    <mat-dialog-actions align="end">
      <button mat-button mat-dialog-close>{{ i18n.t('participants.dialogCancel') }}</button>
...
@Component({
  ...
  template: `
    <section class="participant-management-page stack" data-cy="organizer-participants" aria-labelledby="participant-management-title">
      <a mat-stroked-button class="back-button" routerLink="/organizer/tournaments">{{ i18n.t('participants.back') }}</a>
      @if (loading()) {
        ...
        <div class="error stack" role="alert" data-cy="participant-error">
          <span>{{ error() }}</span>
          <button mat-stroked-button type="button" (click)="load()">{{ i18n.t('participants.retry') }}</button>
        </div>
      } @else if (tournament(); as managedTournament) {
        <header class="page-heading">
          <div><p class="kicker">{{ i18n.t('participants.kicker') }}</p><h1 id="participant-management-title">{{ i18n.t('participants.title', { tournament: managedTournament.title }) }}</h1></div>
        ...
        <p class="muted">{{ i18n.t('participants.scopeHelp', { organization: managedTournament.organizationName }) }}</p>

        <mat-card class="panel"><mat-card-content class="stack">
          <h2>{{ i18n.t('participants.addTitle') }}</h2>
          <p class="muted">{{ i18n.t('participants.lookupHelp') }}</p>
          <form class="auth-form participant-lookup-form" (ngSubmit)="lookupUser()">
            <fieldset [disabled]="!!pending()">
              <label for="participant-lookup-kind">{{ i18n.t('participants.lookupKind') }}</label>
              <select id="participant-lookup-kind" name="lookupKind" [(ngModel)]="lookupKind">
                <option value="username">{{ i18n.t('participants.lookupUsername') }}</option>
                <option value="email">{{ i18n.t('participants.lookupEmail') }}</option>
              </select>
              <label for="participant-lookup-input">{{ lookupKind === 'username' ? i18n.t('participants.lookupUsername') : i18n.t('participants.lookupEmail') }}</label>
              <input id="participant-lookup-input" data-cy="participant-lookup-input" name="lookupValue" [(ngModel)]="lookupValue" autocomplete="off" />
              <button mat-stroked-button type="submit">{{ pending() === 'lookup' ? i18n.t('participants.lookupPending') : i18n.t('participants.lookup') }}</button>
            </fieldset>
          </form>
          @if (lookupError()) { <p class="error" role="alert">{{ lookupError() }}</p> }
          @if (selectedUser(); as user) {
            <div class="participant-selection" data-cy="participant-selection">
              <div><strong>{{ user.username }}</strong><span>{{ user.firstName }} {{ user.lastName }}</span><span>{{ user.email }}</span></div>
        ...
        <section class="stack" aria-labelledby="active-participants-title">
          <h2 id="active-participants-title">{{ i18n.t('registration.participants') }}</h2>
          @if (!participants().length) { <p class="panel participant-empty">{{ i18n.t('participants.empty') }}</p> }
          @else {
            <div class="table-wrap participant-table-wrap" data-cy="participant-table">
              <table class="participant-table" aria-labelledby="active-participants-title">
                <thead><tr><th>{{ i18n.t('participants.username') }}</th><th>{{ i18n.t('participants.legalName') }}</th><th>{{ i18n.t('participants.email') }}</th><th>{{ i18n.t('participants.registeredAt') }}</th><th>{{ i18n.t('participants.status') }}</th><th>{{ i18n.t('participants.actions') }}</th></tr></thead>
                <tbody>@for (participant of participants(); track participant.attemptId) {
                  <tr data-cy="participant-row"><td><strong>{{ participant.username }}</strong></td><td>{{ participant.firstName }} {{ participant.lastName }}</td><td>{{ participant.email }}</td><td>{{ formatDate(participant.registeredAt) }}</td><td>{{ i18n.t('participants.active') }}</td><td><div class="admin-actions participant-actions"><button mat-stroked-button type="button" data-cy="participant-remove" [disabled]="!!pending()" (click)="remove(participant)">{{ i18n.t('participants.remove') }}</button><button mat-stroked-button type="button" data-cy="participant-block" [disabled]="!!pending()" (click)="block(participant, false)">{{ i18n.t('participants.block') }}</button><button mat-stroked-button class="danger-ghost-action" type="button" data-cy="participant-remove-block" [disabled]="!!pending()" (click)="block(participant, true)">{{ i18n.t('participants.removeAndBlock') }}</button></div></td></tr>
                }</tbody>
              </table>
            </div>
            <div class="participant-card-list" role="list">
              @for (participant of participants(); track participant.attemptId) {
                <mat-card class="panel participant-card" role="listitem" data-cy="participant-card"><mat-card-content class="stack">
                  <div><strong>{{ participant.username }}</strong><p>{{ participant.firstName }} {{ participant.lastName }}</p><p>{{ participant.email }}</p></div>
                  <dl><div><dt>{{ i18n.t('participants.registeredAt') }}</dt><dd>{{ formatDate(participant.registeredAt) }}</dd></div><div><dt>{{ i18n.t('participants.status') }}</dt><dd>{{ i18n.t('participants.active') }}</dd></div></dl>
                  <div class="participant-actions"><button mat-stroked-button type="button" data-cy="participant-remove" [disabled]="!!pending()" (click)="remove(participant)">{{ i18n.t('participants.remove') }}</button><button mat-stroked-button type="button" data-cy="participant-block" [disabled]="!!pending()" (click)="block(participant, false)">{{ i18n.t('participants.block') }}</button><button mat-stroked-button class="danger-ghost-action" type="button" data-cy="participant-remove-block" [disabled]="!!pending()" (click)="block(participant, true)">{{ i18n.t('participants.removeAndBlock') }}</button></div>
                </mat-card-content></mat-card>
              }
            </div>
            <nav class="pager" [attr.aria-label]="i18n.t('calendar.pagesAria')"><button mat-stroked-button type="button" [disabled]="participantPage() <= 1 || !!pending()" (click)="goPage(participantPage() - 1)">{{ i18n.t('common.previous') }}</button><span>{{ i18n.t('participants.page', { page: participantPage(), pages: participantPages() }) }}</span><button mat-stroked-button type="button" [disabled]="participantPage() >= participantPages() || !!pending()" (click)="goPage(participantPage() + 1)">{{ i18n.t('common.next') }}</button></nav>
          }
        </section>

        <mat-card class="panel"><mat-card-content class="stack">
          <div class="section-header"><div><h2>{{ i18n.t('participants.blockedTitle') }}</h2><p class="muted">{{ i18n.t('participants.blockedHelp') }}</p></div><button mat-stroked-button type="button" data-cy="blocked-refresh" [disabled]="!!pending()" (click)="loadBlocks()">{{ i18n.t('participants.refreshBlocks') }}</button></div>
          @if (!blockedUsers().length) { <p>{{ i18n.t('participants.noBlocks') }}</p> }
          @else { <div class="participant-block-list" role="list">@for (blocked of blockedUsers(); track blocked.blockId) {
            <div class="participant-block-row" role="listitem" data-cy="blocked-user"><div><strong>{{ blocked.username }}</strong><p>{{ i18n.t('participants.reason') }}: {{ blocked.reason }}</p><p>{{ i18n.t('participants.expires') }}: {{ blocked.expiresAt ? formatDate(blocked.expiresAt) : i18n.t('participants.neverExpires') }}</p></div><button mat-stroked-button type="button" data-cy="participant-unblock" [disabled]="!!pending()" (click)="unblock(blocked)">{{ i18n.t('participants.unblock') }}</button></div>
          }</div> }
        </mat-card-content></mat-card>
        ...
        @if (notificationSettings(); as settings) {
          <mat-card class="panel"><mat-card-content>
            <form class="auth-form" data-cy="participant-notification-settings" (ngSubmit)="saveNotifications(settings)">
              <h2>{{ i18n.t('participants.notificationsTitle') }}</h2><p class="muted">{{ i18n.t('participants.notificationsHelp') }}</p>
              <label class="check-row"><input type="checkbox" data-cy="notify-registration" name="notifyOnRegistration" [(ngModel)]="settings.notifyOnRegistration" /> {{ i18n.t('participants.notifyRegistration') }}</label>
              <label class="check-row"><input type="checkbox" data-cy="notify-unregistration" name="notifyOnUnregistration" [(ngModel)]="settings.notifyOnUnregistration" /> {{ i18n.t('participants.notifyUnregistration') }}</label>
        ...
        @if (actionError()) { <p class="error" role="alert" data-cy="participant-error">{{ actionError() }}</p> }
