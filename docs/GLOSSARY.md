...
| power user | Browser-only opt-in (`gones.settings.power-user`) for Visitor/User advanced mutation UI, forced on without a toggle for Organizer/Admin; never grants server authority or hides browse destinations (ADR 0037) | `src/app/shared/power-user-settings.service.ts` |
