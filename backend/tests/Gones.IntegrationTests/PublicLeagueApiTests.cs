...
public sealed class PublicLeagueApiTests : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        await using (var database = CreateContext())
        {
            ...
            database.LeagueAggregates.Add(LeagueAggregate.Create(FixtureLeague(), Instant.FromUtc(2026, 8, 3, 10, 0)));
            database.LeagueAggregates.Add(LeagueAggregate.Create(new LeagueDocument("deleted", "Deleted", "completed", []), Instant.FromUtc(2026, 8, 3, 9, 0)));
            await database.SaveChangesAsync();
            var deleted = await database.LeagueAggregates.SingleAsync(item => item.DocumentId == "deleted");
    ...
    {
        using var list = await Client.GetAsync("/api/leagues?page=1&pageSize=1&status=active&search=API");
        ...
        using var detail = await Client.GetAsync("/api/leagues/api-league");
        ...
        using var conditional = new HttpRequestMessage(HttpMethod.Get, "/api/leagues/api-league");
        ...
        Assert.Equal(HttpStatusCode.OK, (await Client.GetAsync("/api/leagues/api-league/result")).StatusCode);
        using var tournament = await Client.GetAsync("/api/leagues/api-league/tournaments/result-tournament");
        Assert.Equal("Result Tournament", (await tournament.Content.ReadFromJsonAsync<JsonElement>()).GetProperty("name").GetString());
        using var tournamentResult = await Client.GetAsync("/api/leagues/api-league/tournaments/result-tournament/result");
        ...
        using var stats = await Client.GetAsync("/api/leagues/api-league/players/Alice/statistics?opponentName=Bob");
        Assert.Equal(1, (await stats.Content.ReadFromJsonAsync<JsonElement>()).GetProperty("playedMatchCount").GetInt32());

        Assert.Equal(HttpStatusCode.NotFound, (await Client.GetAsync("/api/leagues/deleted")).StatusCode);
        Assert.Equal(HttpStatusCode.BadRequest, (await Client.GetAsync("/api/leagues?pageSize=101")).StatusCode);
        using var literalWildcard = await Client.GetAsync("/api/leagues?search=API%25");
        Assert.Equal(0, (await literalWildcard.Content.ReadFromJsonAsync<JsonElement>()).GetProperty("totalCount").GetInt32());
        Assert.Equal(HttpStatusCode.BadRequest, (await Client.GetAsync($"/api/leagues/{new string('x', LeagueAggregate.MaximumDocumentIdLength + 1)}")).StatusCode);
    }

    [Fact]
    public async Task Public_exports_project_full_existing_source_without_derived_or_private_data()
    {
        using var leagueExport = await Client.GetAsync("/api/leagues/api-league/export");
