...
- [ ] **The four gates are green.** `npm run test` prints `Test Files 166 passed (166)` and `Tests 2020 passed (2020)` with no failed suite; `npm run typecheck` exits `0` with no output; `npm run lint` prints `All files pass linting.`; `npm run build` exits `0`. The count grew by exactly 122 over T12's 1898 — 8 shell tests and 114 list tests, and nothing else.
...
- [ ] **The rows are two-line, four-column.** On the demo data 12 Seasons render. Each row shows the Season name as a link over its League name; the last-played date over an `upd. …` line; a `… tourn.` count over a `… players` count; and a status chip. Four `<th>` only — if you count more columns, the wrong variant shipped.
...
- [ ] **The Tournaments tab bounces back, on purpose.** Click `Tournaments`. It returns to `/archive/league-seasons`. It must **not** land on the 404 page. This is a temporary redirect placeholder; T14 replaces it with the real tab.
- [ ] **A Season link 404s, on purpose.** Click a Season name. `/archive/league-seasons/<id>` is not registered yet, so the Not Found page is correct here. T14 adds the detail route.
...
- [ ] **Known, accepted — the Synchronize button may make no visible network request.** The archive catalog endpoints send `Cache-Control: public, max-age=3600`, so a forced refetch inside that hour is served from the browser's own HTTP cache and never reaches the network tab. The button is working; the response is simply already local. A hard reload (Ctrl+Shift+R) or DevTools "Disable cache" makes the request visible.
