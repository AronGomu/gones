...
## T23 registration-uniform-answer

Read this first, because it changes what "success" looks like on a screen you have used a hundred times: **`POST /api/auth/register` no longer tells anyone whether an account already exists.** It used to answer `201 Created` with the full new profile for a fresh signup and `409 Conflict` for a taken email or username — which meant anyone could type an address into the register form and read back, for free, whether that person has an account here. It now answers `202 Accepted` with the same generic body in both cases: `{"message":"If the account is eligible, an email has been queued."}`. `201` and `409` are gone from that route entirely. When the duplicate belongs to an account that has **not** confirmed its email, a fresh verification email is queued for the real owner out of band — the same thing `POST /api/auth/resend-verification` does — so a genuine person who re-registers because they lost the first email still gets in. When the duplicate belongs to an already-confirmed account, nothing is queued and nothing is said. Validation failures (bad email, weak password, over-long username) still answer `400` `validation_failed` exactly as before, because those do not depend on whether an account exists.

The whole point of this slice is that **the two answers must be indistinguishable**. So the central test below is a two-sided one, and it is worth doing with your own eyes rather than trusting the suite. Bring the stack up with `npm run dev` (API on `http://127.0.0.1:5080`, app on `http://127.0.0.1:4200`) and keep DevTools → Network open on the app tab throughout.

- [ ] **The two-sided check — a known-existing address and a fresh one give byte-identical answers.** This is the ticket. Register a first account through the UI at `/register` (use `t23-known@example.test`, any username, password `valid-password-value`) and let it land on `/verify-email`. Now run both halves back to back from a terminal, using `-i` so you see the status line and every header:<br>`curl -isS -X POST http://127.0.0.1:5080/api/auth/register -H 'content-type: application/json' -d '{"email":"t23-known@example.test","username":"t23dup","password":"valid-password-value","firstName":"Dup","lastName":"User"}' > /tmp/t23-known.txt`<br>`curl -isS -X POST http://127.0.0.1:5080/api/auth/register -H 'content-type: application/json' -d '{"email":"t23-fresh@example.test","username":"t23fresh","password":"valid-password-value","firstName":"Fresh","lastName":"User"}' > /tmp/t23-fresh.txt`<br>Both files must start `HTTP/1.1 202 Accepted` and both must end with exactly `{"message":"If the account is eligible, an email has been queued."}`. Then diff them ignoring only the two headers that carry a fresh value on every request regardless of outcome: `diff <(grep -viE '^(date|x-correlation-id):' /tmp/t23-known.txt) <(grep -viE '^(date|x-correlation-id):' /tmp/t23-fresh.txt)` → **no output**. In particular neither response may carry a `Location` header (the old `201` did: `Location: /api/users/me`), and there must be no `problem+json` body, no `code` field and no `409` anywhere. If the diff prints anything at all, that difference is the oracle this ticket exists to close — read it before doing anything else.
- [ ] **The duplicate really did not create a second account.** After the step above, `docker compose exec -T postgres psql -U gones -d gones -c "select count(*) from asp_net_users where normalized_email = 'T23-KNOWN@EXAMPLE.TEST'"` must print `1`, and the same query against `user_profiles` joined on that user must show a single profile. The username `t23dup` you tried to claim must exist nowhere: `select count(*) from user_profiles where normalized_username = 'T23DUP'` → `0`. A generic answer that quietly created a half-account would be worse than the oracle it replaced.
- [ ] **The unverified duplicate re-sent the verification email to the real owner.** `t23-known@example.test` was never confirmed, so the duplicate attempt above should have queued a *new* verification link for it. Check the worker's file sink: `docker compose exec -T worker sh -ec "find /tmp/gones-email-sink -maxdepth 1 -type f -name '*.json' -exec cat {} \;" | grep -c t23-known` must have grown by one since before the duplicate call, and the newest of those entries must be a `verify-email` template addressed to `t23-known@example.test`. In the database, `select count(*) from account_action_tokens t join asp_net_users u on u.id = t.user_id where u.normalized_email = 'T23-KNOWN@EXAMPLE.TEST' and t.purpose = 'VerifyEmail'` → `2`, and exactly one of those rows has `superseded_at is null`. Open the newest link and confirm it verifies the account — the re-send must be usable, not decorative.
- [ ] **A duplicate for an already-confirmed account queues nothing.** With `t23-known@example.test` now confirmed (previous step), note the current sink file count, then repeat the duplicate `curl` from step 1 verbatim. The answer must again be `202` with the same body — identical to before — but **no new email may appear in the sink**, and `select count(*) from notification_outbox_records o join asp_net_users u on u.id = o.user_id where u.normalized_email = 'T23-KNOWN@EXAMPLE.TEST'` must not have grown. This is the asymmetry that is allowed to exist: it is invisible over HTTP, and only the real mailbox owner can observe it.
- [ ] **Validation still answers `400`, and still names the field.** At `/register` submit an obviously bad password (`short`) and a malformed email (`not-an-email`). Both must produce `400` with `application/problem+json`, `"code":"validation_failed"`, and the field errors bound to the right inputs — the email input gets `aria-invalid="true"` and `aria-describedby="register-email-error"`, and the message renders under it. Nothing here may have turned into a `202`: a validation failure is not an existence-dependent condition, so it keeps its old, specific answer.
- [ ] **The register form still lands on Verify Email with the address you typed.** This changed shape internally: the app used to read the email out of the `201` profile body, and now reads it from the form, because the `202` body has no email in it. At `/register` type a fresh address, submit, and confirm the URL is `/verify-email?email=<exactly what you typed>&registered=true` and that `[data-cy="verify-email-address"]` is pre-filled with that same address. Do it once with an address containing a `+` (`t23-plus+tag@example.test`) to confirm the query-parameter encoding survives, and confirm the resend button on that page works.
- [ ] **A safe `returnUrl` still survives registration end to end; an unsafe one is still dropped.** Visit `/register?returnUrl=%2Fevents%3Fmonth%3D2026-08%26view%3Dlist%26register%3Dlyon-legacy` and register a fresh account: the redirect must be `/verify-email?email=…&registered=true&returnUrl=/events?month=2026-08&view=list&register=lyon-legacy`, and the verification email in the sink must carry that same `returnUrl` on its action link. Then visit `/register?returnUrl=https%3A%2F%2Fevil.test%2Fsteal` and register another fresh account: the redirect must carry **no** `returnUrl` at all, and `evil.test` must appear nowhere in the queued email.
- [ ] **Observability still distinguishes the two paths, deliberately.** The oracle closes at the HTTP boundary only — operators must still be able to see a duplicate. After a duplicate attempt, `docker compose exec -T postgres psql -U gones -d gones -c "select action, redacted_diff from audit_records where action like 'auth.register%' order by occurred_at desc limit 3"` must show `auth.register.failed` with `redacted_diff` exactly `{"outcome":"conflict"}` for the duplicate, and `auth.register.succeeded` for the fresh one. Note that the audit row for a duplicate has a null actor and names no email, so the audit trail records *that* a collision happened without recording *whose*.
- [ ] **The OpenAPI contract and the generated client agree with the server.** `git show HEAD -- backend/openapi/gones.json` must show `/api/auth/register` losing its `201` and `409` blocks and gaining a `202` that references `GenericAccountActionResponse`, leaving `202`/`400`/`429` as the only three responses. In `src/app/api/generated/gones-api.ts` the signature must read `register(body: RegisterRequest): Observable<GenericAccountActionResponse>` and `processRegister` must branch on `status === 202` with no `409` branch left. Re-running `npm run api:check` (with `GONES_DB_CONNECTION` pointed at your local Postgres) must exit `0`, proving the committed snapshot is what the running server actually produces.
- [ ] **Known residual — timing is NOT covered by this ticket, and was not measured.** The duplicate path does more work than a rejected-validation path and different work from the fresh path (it rolls back a transaction, writes an audit row, then may open a second transaction to issue a token and queue an email). A determined attacker with a low-noise network could plausibly still separate "existing unverified account" from "fresh address" by response time. Nothing in this commit attempts to equalise that, and no timing measurement was taken. Status code, problem code, response body and response headers are uniform and are proven so by `Normalized_email_collision_answers_the_same_generic_202_and_username_collision_names_the_field`; **timing is explicitly out of scope**. Treat a future timing-oracle finding as new work, not as a regression of this one.
- [ ] **Known residual — the rate limiter is still keyed on the submitted email.** `AuthAccountRateLimitFilter` partitions on `Email`, so hammering one address can produce `429` on a per-account budget while a different address still gets `202`. That behaviour predates this ticket and was explicitly out of its scope (`429` is unchanged), but it is a per-address signal reachable from the same endpoint, so it is worth knowing about while reasoning about the oracle as a whole.
- [ ] **Known residual — an unlikely failure inside the out-of-band resend would surface only on the duplicate path.** `TryResendVerificationAsync` runs after the registration transaction is rolled back, and the ticket froze that shape verbatim. If it ever threw a `DbUpdateException`, the outer `catch (DbUpdateException)` would try to roll back an already-completed transaction and the caller would see a `500` — which only a duplicate could trigger, i.e. a fault-shaped oracle. In practice the path takes a `SELECT … FOR UPDATE` on the user row first, which serialises concurrent duplicates for the same address, and the new token's outbox dedupe key is a fresh GUID, so there is no observed way to provoke it. Flagged rather than redesigned, because the block was frozen by the plan.
- [ ] **No e2e ran for this slice.** `npm run e2e:ci` and `npm run cy:run` were deliberately not executed (their teardown destroys local database volumes). Two Cypress specs touch this route: `cypress/e2e/event-registration.cy.js` had its register stub updated from `201` + profile to `202` + generic body and was **not** run, and `cypress/e2e/auth-profile.cy.js` was deliberately left untouched — its register intercept stubs `400` (an unchanged path) and its live-API test asserts only that registration lands on `/verify-email` with the typed address, which this change preserves. At the end-of-plan `npm run e2e:ci`, read any failure in either of those two specs against this commit first.
- [ ] **The commit is the ticket's files plus three the plan missed and this checklist.** `git show --stat HEAD` must list `backend/src/Gones.Api/Identity/LocalIdentityEndpoints.cs`, `backend/src/Gones.Api/Identity/AccountLifecycleEndpoints.cs`, `backend/openapi/gones.json`, `src/app/api/generated/gones-api.ts`, `src/app/auth/auth.service.ts`, `src/app/auth/auth-entry.component.ts`, `src/app/auth/auth-entry.register.test.ts`, the deletions of `src/app/auth/registration-gate.ts` and `src/app/auth/registration-gate.test.ts`, nine `backend/tests/Gones.IntegrationTests/*.cs` files, `scripts/smoke-account-lifecycle.mjs`, `deploy/release-test/journeys.mjs`, `cypress/e2e/event-registration.cy.js`, `ops/acceptance-matrix.json`, and `artifacts/manual_test_checklist.md`. Three of those were **not** in the ticket's `## Outputs` and were added under a supervisor ruling: `AdminBootstrapAndCatalogTests.cs` and `OrganizationApiTests.cs` (four `HttpStatusCode.Created` → `Accepted` asserts on the register response that the plan's own grep missed, because one of them names its variable `unverifiedReg` rather than `registration`), and `ops/acceptance-matrix.json` (two evidence rows that pointed at a deleted test file and at an assertion string the ticket itself rewrote). No migration and no compose/config change: `git show HEAD -- backend/src/Gones.Infrastructure/Persistence/Migrations/ deploy/compose/` must print nothing.

## T24 backup-hmac-strict

**Read this first and act on it today: every encrypted backup taken before this commit can no longer be restored.** Restore is now strict — `deploy/backup/gones-restore.sh` refuses any archive that is not accompanied by a valid `.hmac` sidecar, and archives written by the old script have no sidecar at all. There is deliberately no legacy flag, no grace window and no compatibility path: that was a locked decision, because any escape hatch re-opens exactly the hole this closes. **The action this requires from you is one line: rebuild the backup image from this commit, take a fresh backup, and keep it.** Until you do that, you are running without a usable restore point. Nothing in the application is affected — no runtime code, no schema, no API changed; this touches only the two backup shell scripts, their rehearsal, and the docs that describe them.

What changed, so the steps below make sense. `gones-backup.sh` now writes a fourth file next to the archive, `<name>.dump.enc.hmac`, holding exactly one line: `v1 pbkdf2-sha256-600000 <16 hex chars of salt> <64 hex chars of MAC>`. The MAC is HMAC-SHA256 over the **ciphertext**, keyed by a second PBKDF2 key derived from the same `GONES_BACKUP_KEY`/`GONES_BACKUP_KEY_FILE` passphrase with its own random salt — no new secret, no new environment variable, and a different key from the one that encrypts. `gones-restore.sh` verifies that MAC **before it decrypts anything**: the verification sits at `deploy/backup/gones-restore.sh:52-60` and `openssl enc -d` is at `:64`, so attacker-supplied bytes are never fed to the decrypter unauthenticated, and the database is never contacted on any refusal path. The full check order is path guards (exit `2`) → `.sha256` (exit `10`) → `.hmac` (exit `12`) → decrypt (exit `11`) → `PGDMP` magic (exit `11`) → database. **A wrong passphrase now exits `12` where it used to exit `11`**, because the MAC key is derived from the passphrase and fails first; `11` survives only for the pathological case where the MAC verifies but the payload is not a `pg_dump` archive.

One thing to know before you start: the scripts are baked into the image (`deploy/backup/Dockerfile` `COPY`s them at build time), so a deployed backup container keeps the old MAC-less behaviour until the image is rebuilt from this commit. Taking a "fresh backup" with a stale image just produces another unrestorable archive — that exact trap cost this ticket a failed rehearsal run, which is why `scripts/backup-restore-rehearsal.mjs` now builds the tools image explicitly before it asserts on its behaviour.

- [ ] **Rebuild the backup image, then take a fresh backup — do this before anything else.** Build from this commit (`npm run images:build`, or your own `docker build -f deploy/backup/Dockerfile .`) and confirm the new code is actually inside the artifact before trusting it: `docker run --rm --entrypoint sh <your-backup-image> -c 'grep -c "mac HMAC" /usr/local/bin/gones-backup.sh /usr/local/bin/gones-restore.sh'` must print `1` for both files, not `0`. Then run the backup exactly as your scheduler does, with the real `GONES_BACKUP_ROOT` mount and the real `GONES_BACKUP_KEY_FILE`. The run must exit `0` and the mounted root must now hold **four** files for that name, not three: `<name>.dump.enc`, `<name>.dump.enc.sha256`, `<name>.dump.enc.hmac`, `<name>.meta.json`. If only three appear, you are still running the old image — stop and rebuild.
- [ ] **The sidecar has the documented shape, and the metadata records the algorithm.** `cat <name>.dump.enc.hmac` must print exactly one line of four space-separated fields: the literal `v1`, the literal `pbkdf2-sha256-600000`, a 16-character lowercase hex salt, and a 64-character lowercase hex MAC. Anything else — a wrapped line, an empty file, a different label — will be rejected by restore with exit `12`, by design. `cat <name>.meta.json` must now carry `"mac": "hmac-sha256/pbkdf2-sha256-600000"` alongside the existing `"cipher"` and `"checksum"` keys.
- [ ] **Restore that fresh backup and watch it succeed.** Against a scratch database (never the live one), run the restore from the same rebuilt image with the same passphrase: exit `0`, and the data is there. This is the step that proves the two sides agree — the backup's MAC key and the restore's MAC key are derived independently from the same passphrase, so a successful restore is also proof that the derivation is deterministic across runs and across the `env:`/`file:` forms of the secret. If this fails, do not delete anything: report it before touching the old archives.
- [ ] **An old, pre-MAC archive is refused with a message that tells you what to do — and nothing is decrypted.** Point the restore at one of your existing pre-commit archives (or reproduce it in seconds: `mkdir -p /tmp/t24 && chmod 0777 /tmp/t24 && printf 'Salted__pretend-this-is-an-old-archive' > /tmp/t24/legacy.dump.enc && chmod 0666 /tmp/t24/legacy.dump.enc && docker run --rm -v /tmp/t24:/backups --entrypoint sh <your-backup-image> -c 'cd /backups && sha256sum legacy.dump.enc > legacy.dump.enc.sha256'`, then `docker run --rm -v /tmp/t24:/backups -e GONES_BACKUP_KEY=any-passphrase -e GONES_BACKUP_DSN=postgresql://unused --entrypoint /usr/local/bin/gones-restore.sh <your-backup-image> legacy.dump.enc; echo "exit=$?"`). It must print exactly `gones-restore: no legacy.dump.enc.hmac next to the archive, refusing to restore an unauthenticated archive` and exit `12`. Read that outcome carefully: it is a refusal, not a crash and not a silent empty restore — note that the DSN above is deliberate nonsense and was never used, because the refusal happens before any database contact.
- [ ] **A malformed sidecar is refused too, rather than being ignored.** With the same throwaway archive, write junk into the sidecar and re-run the restore for each of: `garbage`, an empty file, `v2 pbkdf2-sha256-600000 0011223344556677 deadbeef`, and a truncated `v1 pbkdf2-sha256-600000`. Every one must exit `12` with `gones-restore: unsupported MAC format in legacy.dump.enc.hmac, refusing to restore`. Then write a well-formed line with a wrong MAC (`v1 pbkdf2-sha256-600000 0011223344556677` followed by 64 zeroes) — that one must exit `12` with `gones-restore: MAC verification failed: legacy.dump.enc was tampered with or the key is wrong, refusing to restore`. No openssl decryption error may appear in any of these, which is the observable proof that the MAC gate runs before the decrypter.
- [ ] **The substitution attack this ticket exists to stop is actually stopped.** Copy a real archive and its `.hmac`, flip a byte in the copy, then regenerate its `.sha256` so the checksum sidecar agrees with the tampered bytes — the exact move an attacker with write access to the backup directory would make. Restore must still exit `12` with `MAC verification failed`. Before this commit that restore would have succeeded and replayed attacker-chosen data into your database. `npm run backup:rehearsal` performs this whole case automatically (`=== a tampered archive with a regenerated checksum must still be refused ===`), and is the fastest way to see it end to end.
- [ ] **A wrong passphrase now exits `12`, not `11` — update any alerting that matches on the old code.** If you have monitoring, a runbook, or a wrapper script that keys off restore exit codes, `11` no longer means "wrong key" in the normal case. `10` (corrupt/missing checksum) and `2` (bad path or bad configuration) are unchanged. `docs/OPERATIONS.md` §7 and `docs/RUNTIME_CONTRACT.md` "Backup scheduler" both carry the new table.
- [ ] **Decide what to do with the now-unrestorable archives — deliberately, not by accident.** Every pre-commit archive in your backup root is dead weight: it cannot be restored and never will be. That is the intended consequence of the strict decision, but deleting backup data is irreversible and is explicitly **your** call, not the deployment's — nothing in this commit deletes or rewrites anything in the backup root. Keep them if they have audit value; if you retire them, do it only after the fresh backup above has been restored successfully at least once.
- [ ] **Known residual — the MAC comparison is a plain shell string compare, not a constant-time one.** The verification is `[ "$mac_actual" = "$mac_expected" ]`, frozen verbatim by the ticket; POSIX `sh` has no fixed-time comparison primitive, and the alternative (blinded double-HMAC comparison) would have been a redesign rather than an implementation. The theoretical leak is a timing side channel on the comparison of a 64-character hex string — to exploit it an attacker would need to repeatedly trigger restores and measure them through PBKDF2 at 600k iterations, a full-archive HMAC, and container startup, all of which dwarf the signal by many orders of magnitude, and restore is an operator-initiated action rather than an attacker-reachable oracle. Recorded here so a future reviewer sees it was a decision, not an oversight.
- [ ] **Known residual — the encryption itself is still AES-256-CBC, an unauthenticated mode.** This commit authenticates the ciphertext with a separate MAC (encrypt-then-MAC, verified before decrypt) rather than switching to an AEAD mode, because switching the cipher would change the envelope format. The security property is equivalent for this threat model; the reason it is worth knowing is that anything reading these archives outside `gones-restore.sh` gets no such protection.
- [ ] **The commit is the ticket's six files plus this checklist.** `git show --stat HEAD` must list exactly `deploy/backup/gones-backup.sh`, `deploy/backup/gones-restore.sh`, `scripts/backup-restore-rehearsal.mjs`, `ops/image-contract.test.ts`, `docs/OPERATIONS.md`, `docs/RUNTIME_CONTRACT.md` and `artifacts/manual_test_checklist.md`. No `.cs`, no Angular source, no migration, no compose file and no `deploy/backup/Dockerfile` change: the image rebuilds from the same Dockerfile, it just picks up the two edited scripts. `npm run e2e:ci` and `npm run cy:run` were deliberately not run for this slice (their teardown destroys local database volumes) and no Cypress spec touches the backup path.

## T25 sort-indicator-match

Audit finding F29. `/global-stats` reads its sort from the URL through a gate: `?sort=decayedRating`
is only honoured once the loaded rows prove the server is exposing the `decayedRating` column, because
the endpoint answers `400` for that sort while `Gones:PlayerStatistics:ExposeDecayedRating` is off
(`backend/src/Gones.Api/Archive/ArchivePlayerStatisticsEndpoints.cs:190`). The bug was an ordering
problem in the client: the first request was built **before any rows existed**, so the gate was still
shut and the request went out unsorted — then the rows landed carrying `decayedRating`, the gate
opened, and the Decayed header started advertising `aria-sort="descending"` over a table that was
still in default ranking order. The indicator lied until the visitor navigated again. The component
now compares the sort it actually sent against the sort the URL asks for once a response lands, and
re-issues the request **once** when they disagree, under the same spinner — so the mismatched frame is
never painted. A one-shot flag reset on every query-parameter change makes two requests the hard
ceiling per navigation. Nothing about the wire, the routes or the visual design changed.

Note for whoever runs this: the down/up arrow on a sortable header is pure CSS keyed on the attribute
itself (`.ranking-table th[aria-sort="descending"]::after { content: ' ↓'; }`), so the arrow you see
and the ARIA state a screen reader announces are the same fact — checking one checks both, and they
cannot drift apart. Run everything from the repository root with `npm run dev -- --detached` (API on
`http://127.0.0.1:5080`, app on `http://127.0.0.1:4200`), seeded via `npm run dev:accounts` and
`npm run dev:env`. Do **not** run `docker compose down --volumes` or `npm run db:reset` at any point.

- [ ] **First, make the case reachable — the switch is off by default and dev compose does not pass it.** The `api` service's `environment:` block in `compose.yaml` carries no `GONES_PLAYER_STATISTICS__*` entry, so exporting the variable in your shell will not reach the container. Write an override — `printf 'services:\n  api:\n    environment:\n      GONES_PLAYER_STATISTICS__EXPOSE_DECAYED_RATING: "true"\n' > /tmp/t25-decayed.yaml` — and restart just the API with it: `docker compose -f compose.yaml -f /tmp/t25-decayed.yaml up -d api`. Confirm it took effect against the wire, not against the config: `curl -sS "http://127.0.0.1:5080/api/archive/global-player-statistics?scopeKind=global" | jq '[.items[].decayedRating]'` must print numbers, not a list of `null`. **If every value is `null` the gate can never open and the rest of this section is untestable** — the column is computed during the statistics rebuild, so resync the archive first and re-check. Keep this override in place for every step below except the flag-off one.
- [ ] **The finding itself: a cold load must never show an indicator for an ordering the table does not have.** Open a brand-new tab (or hard-reload with DevTools → Disable cache ticked) directly on `http://127.0.0.1:4200/global-stats?sort=decayedRating&dir=desc`. **Do not click anything.** The moment the table appears, read the Decayed Rating header and the rows together: the header must carry the ` ↓` arrow **and** the rows must actually descend by the Decayed value (top row highest, provisional players kept at the bottom, any `—` last). Before this commit the arrow was there and the ordering underneath it was the default ranking — an arrow over rows it did not describe. Check the ARIA state matches what you see, in the console: `document.querySelector('[data-cy="global-stats-col-decayed-rating"]').getAttribute('aria-sort')` must return exactly `descending`, and no other `th` may return anything but `null` (`[...document.querySelectorAll('th[aria-sort]')].map(t => [t.dataset.cy, t.getAttribute('aria-sort')])` — exactly one non-null entry).
- [ ] **Two requests, and the second one is the sorted one.** Repeat that cold load with DevTools → Network filtered on `global-player-statistics`. Exactly **two** requests must appear, in this order: the first with **no** `sort` parameter at all, the second with `sort=decayedRating&direction=desc`. Two is the ceiling — a third would mean the one-shot guard is broken. Also confirm the response of the second is what the table is rendering (its first item's `playerName` matches the top row on screen).
- [ ] **The unsorted table must never flash on screen.** Same cold load, but throttle DevTools → Network to "Slow 3G" first so both round trips are slow and visible. Watch the page across the whole load: you must see the loading state, then the finished sorted table — **never** a fully-rendered table that then re-orders itself. The table only renders when loading has finished, and the retry deliberately keeps the spinner up across both requests. A visible re-sort is a regression in this slice. Repeat once with the throttle off to be sure you did not simply miss it.
- [ ] **With the switch off, everything behaves exactly as it did before.** Restart the API without the override (`docker compose -f compose.yaml up -d api`) and load the same URL `http://127.0.0.1:4200/global-stats?sort=decayedRating&dir=desc` cold. The Decayed Rating column must be **absent** from the table entirely, no header may carry an `aria-sort` attribute (the console one-liner above returns an empty array), and the Network tab must show exactly **one** request, carrying no `sort` parameter. The client refuses the sort for the same reason the server would `400` it. Put the override back before continuing.
- [ ] **An ordinary, ungated sort is untouched — one request, as always.** With the override back on, cold-load `http://127.0.0.1:4200/global-stats?sort=matchWins&dir=asc`. Exactly **one** request, carrying `sort=matchWins&direction=asc`; the Wins header shows ` ↑` and reports `ascending`; the rows ascend by Wins. Then exercise the page normally end to end — click several headers to toggle direction, page forward and back, change the page size, type in the search box, and narrow with the League and Season selects. Every one of those is a fresh navigation and must still cost exactly one request. The retry path is dead code whenever the URL's sort and the sent sort already agree, which is every case except the gated first load.
- [ ] **Known residual — the one-shot guard resets on navigation, not on the Sync button.** `onSync()` re-runs the request without clearing the retry flag, so in one very specific sequence the old mismatch can still appear: load with `?sort=decayedRating` while the switch is on (which spends the retry), then have the server's answer stop carrying the column, then have it start carrying it again — all across two manual **Sync** presses without navigating. The indicator would then advertise a sort the latest request did not carry, until the next navigation heals it. Reaching it requires the feature switch to be flipped twice server-side between two button presses in one page session; the ticket scoped the reset to query-parameter changes deliberately, so this was left exactly as specified rather than widened. Worth knowing, not worth chasing.
- [ ] **Known residual — a failing retry can leave a stale error banner up.** The re-issued request returns early before the success path clears `error`, so if a *previous* navigation had failed with an empty table and the retry then also fails, the old error message stays on screen instead of the stale-data banner. The outcome is still honest (the sorted data genuinely did not load) and the pre-existing `catch` handles it with no new error string, which is what the ticket specified. No action unless you see an error banner that outlives a successful load.
- [ ] **The commit is the two files the ticket names plus this checklist.** `git show --stat HEAD` → `src/app/features/players/global-stats.component.ts`, `src/app/features/players/global-stats.component.test.ts`, `artifacts/manual_test_checklist.md`. No backend file, no migration, no config, no i18n and no Cypress spec: `git show HEAD -- backend/ cypress/ ops/ src/app/i18n/` must print nothing. `npm run e2e:ci` and `npm run cy:run` were deliberately not run for this slice (their teardown destroys local database volumes); no Cypress spec needed repair because `cypress/e2e/global-stats.cy.js` fixes `decayedRating: null` on every row (`:27`), so the gate never opens there and the request count it waits on is unchanged.

## T26 local-restore-refuse-orphans

Read this first, because this commit makes an import that used to "work" stop working — on purpose. A v5 bundle can name links inside itself: every `leagueSeasons[].leagueId` points at a `leagues[].id`, and every non-null `tournaments[].seasonId` points at a `leagueSeasons[].id`. The server has always refused a bundle whose link points at nothing (`ArchiveTournamentCommandEndpoints.cs`, HTTP `400`, validation field `leagueSeasons` or `tournaments`, message `Bundle link does not resolve inside the bundle.`). The browser-local path did not: it accepted the file and quietly rewrote the dangling link — an orphan Season landed with an empty `leagueId`, an orphan Tournament landed as if it were standalone (`seasonId: null`). Those rows were unreachable rot: the browser-local create path can never produce them, the League filter cannot see the Season, and no screen offers to repair or delete it. From this commit both the import gate (`parseArchiveBundle`) and the local restore (`LocalArchiveBackend.restoreArchiveBundle`) refuse such a file before anything is written, with `unresolvedArchiveBundleLink:leagueSeasons` / `unresolvedArchiveBundleLink:tournaments`.

Two things need your eyes: the refusal has to be **visible and total** (a message on screen, zero rows written, no half-import), and a **normal bundle has to keep importing exactly as before**. Work signed out (or as a plain `User`) with the power-user setting on, so the archive target is the browser-local store — signed in as an Organizer/Admin the same file goes to the server, which already refused it. Run from the repository root with `npm run dev` (API on `http://127.0.0.1:5080`, app on `http://127.0.0.1:4200`). Do **not** run `docker compose down --volumes` or `npm run db:reset` at any point. Keep DevTools → Application → IndexedDB → `gones-archive-local` open throughout; its three stores (`leagues`, `league-seasons`, `tournaments`) are the observation point.

- [ ] **Make a known-good bundle first.** Signed out with the power-user setting on, go to `/archive/league-seasons`, create a League (`Lyon`), a Season under it (`Spring`) and a Tournament inside that Season (`Weekly`), plus one standalone Tournament with no Season (`Open`). Use the header **Export** action to download the JSON. Open the file: it must contain one `leagues[]` row, one `leagueSeasons[]` row whose `leagueId` equals the league's `id`, and two `tournaments[]` rows — one whose `seasonId` equals the season's `id`, one with `"seasonId": null`. Keep this file; every step below starts from a copy of it.
- [ ] **The control: the good bundle still imports, unchanged.** In a fresh profile (or after clearing `gones-archive-local` via DevTools → right-click the database → Delete database, then reload), click **Import league(s)** (`data-cy="app-leagues-import-button"`) and pick the untouched export. It must succeed with no error banner: `/archive/league-seasons` shows the League with its Season, `/archive/tournaments` shows both Tournaments, and the standalone one still shows **no** Season. In IndexedDB the restored rows carry freshly minted `local-…` ids (not the ids inside the file) and the Season's `leagueId` equals the *new* League row's `id`. This is the behaviour that must not have changed — if it did, stop here, nothing below matters.
- [ ] **The finding, side one: a Season pointing at an absent League is refused.** Copy the good export to `orphan-season.json`, then make exactly two edits: set `leagueSeasons[0].leagueId` to `"no-such-league"`, and **delete the whole `"checksum"` line**. The checksum is verified before the bundle is parsed (`archive-import.service.ts`), so a file you hand-edited but left checksummed is refused earlier for the wrong reason — a bundle carrying no checksum at all is accepted and reaches the check this step is about. Clear the database again, reload, and import that file. A red banner must appear (`role="alert"`, `data-cy="app-import-error-banner"`) reading **"Could not import that Gones Export. Please try again."** — in French, **"Impossible d’importer cet export Gones. Réessayez."**. The page must stay usable: no blank screen, no spinner stuck on, the Import button enabled again.
- [ ] **The refusal is total — nothing was written.** Straight after that failed import, look at IndexedDB → `gones-archive-local`: all three stores (`leagues`, `league-seasons`, `tournaments`) must be **completely empty**. Not "missing the Season" — empty. The League and both Tournaments in that same file must not have landed either; the check runs before the write transaction opens, so it is all-or-nothing. Reload the page and confirm `/archive/league-seasons` and `/archive/tournaments` both show their empty state.
- [ ] **The finding, side two: a Tournament pointing at an absent Season is refused the same way.** Copy the good export to `orphan-tournament.json`, set the *seasoned* tournament's `seasonId` (the one that is not `null`) to `"no-such-season"`, drop the `"checksum"` line, clear the database, reload and import. Same banner, same emptiness in all three stores. Then make one more copy where you only touch the **standalone** tournament — leave its `"seasonId": null` exactly as it is — and confirm that file still imports fine. `null` means "standalone" and is first-class valid; only a non-null id that resolves to nothing is refused.
- [ ] **The exact refusal reason is in the console, not on screen.** Repeat the orphan-season import with the DevTools Console open. Alongside the banner you must see the structured boundary error for `app-header.importArchive` carrying the message `unresolvedArchiveBundleLink:leagueSeasons` (the tournament case reads `unresolvedArchiveBundleLink:tournaments`). That suffix is the field scoping the server uses — the browser now refuses on the same rule, with the same two field names.
- [ ] **Server parity — the same file, the same verdict.** Sign in as an Admin (server target) and import `orphan-season.json` there. The server must refuse it too, with HTTP `400` in DevTools → Network on the `restore` call and the validation message `Bundle link does not resolve inside the bundle.` scoped to `leagueSeasons`. The point of this commit is that the two halves of the app now agree: before it, this same file was accepted locally and rejected on the server.
- [ ] **Round-trip stays honest.** Back signed out, with the good bundle imported, export again and diff the new file against the original (`diff <(jq -S . first.json) <(jq -S . second.json)`): only the ids and the `checksum` may differ (the restore mints fresh `local-…` ids by design); the structure, names, dates, statuses, rounds and archetypes must match one for one, and every link in the new file must still resolve inside it.
- [ ] **Known residual — orphan rows already in your browser are not repaired.** This commit blocks new orphans at the door; it does nothing about a Season with an empty `leagueId` that an earlier import already wrote into `gones-archive-local`. Such a row still renders under an unknown parent and still has no UI to fix or delete it. If you find one on a machine that imported a bad file before this commit, the only cure is deleting the row by hand in DevTools (or deleting the database). Repair affordances were explicitly out of scope for this slice.
- [ ] **Known residual — the banner text is generic, by design.** The refusal deliberately falls through to the existing `msg.importFailed` copy rather than getting its own sentence, exactly like every other `unsupportedArchiveBundle` refusal today. So the screen says "Could not import that Gones Export" without naming the dangling link; the specific reason is in the console message above. No new i18n key was added in either catalog.
- [ ] **The commit is the four files the ticket names plus this checklist.** `git show --stat HEAD` → `src/app/domain/archive-export-schemas.ts`, `src/app/domain/archive-export-schemas.test.ts`, `src/app/backend/local-archive-backend.service.ts`, `src/app/backend/local-archive-backend.service.test.ts`, `artifacts/manual_test_checklist.md`. No server change: `git show HEAD -- backend/ cypress/ ops/ src/app/i18n/` must print nothing — the server already refused correctly and its strings are untouched. `npm run e2e:ci` and `npm run cy:run` were deliberately not run for this slice (their teardown destroys local database volumes); no Cypress spec exercises this path (`cypress/e2e/league-local.cy.js` seeds IndexedDB directly and never calls the restore).

## T27 full-export-error-path

This commit gives the header's **Export** action the error path it never had. Before it, pressing `data-cy="app-full-data-export-button"` while the browser store could not be opened did *nothing at all*: `downloadFullExport()` had no `try`, Angular's `(click)` discards the promise it returns, so the rejection from `openDatabase` (`src/app/backend/indexed-db.ts`, one of `indexedDbUnavailable` / `indexedDbOpenFailed` / `indexedDbBlocked`) became an unhandled rejection nobody surfaced. The button looked dead. From this commit the same failure logs a boundary error and paints the existing red banner (`data-cy="app-import-error-banner"`, `role="alert"`) with a new message. Two things need your eyes: the failure has to be **visible** (banner on screen, error still in the console), and the **success path must be untouched** — the file that downloads today must be identical to the one that downloaded before.

Work **signed out** with the power-user setting on, so the archive target is the browser-local store; signed in, the button refuses before it ever reaches the store. Run from the repository root with `npm run dev` (API on `http://127.0.0.1:5080`, app on `http://127.0.0.1:4200`). Do **not** run `docker compose down --volumes` or `npm run db:reset` at any point. The button only exists on `/archive/league-seasons`, so stay on that route.

- [ ] **Baseline first — capture the good export before you break anything.** Signed out with the power-user setting on, go to `http://127.0.0.1:4200/archive/league-seasons`, create a League, a Season under it and a Tournament inside that Season if the store is empty. Press **Export** (`data-cy="app-full-data-export-button"`). A file named `YYYY-MM-DD Gones Archive.json` must download, no banner may appear, and the file must carry `"version": 5` and a `"checksum": "sha256:…"`. Keep this file — the last step diffs against it.
- [ ] **Force the store-open failure — Firefox, the deterministic way.** Open `about:config`, accept the warning, set `dom.indexedDB.enabled` to `false`, then **reload** the app tab (the flag is read when the page opens the database, and the app memoises a successful handle, so a reload is required — toggling the flag in a tab that already exported will not reproduce). Confirm the kill switch took: in the console, `window.indexedDB` must print `undefined`. Navigate to `/archive/league-seasons` and press **Export**.
- [ ] **Force the store-open failure — Chrome/Chromium, or as a second opinion.** Open `chrome://settings/content/siteData`, choose **"Don't allow sites to save data on your device"** (adding `127.0.0.1:4200` to the *Not allowed* list works too), then reload the app tab and press **Export**. Chromium refuses the `indexedDB.open()` rather than removing the API, so this exercises the `indexedDbOpenFailed` branch instead of `indexedDbUnavailable` — the user-facing outcome must be identical.
- [ ] **What you must see: the banner, not silence.** Under the toolbar a red banner (`data-cy="app-import-error-banner"`, `role="alert"`) must read, in English, **"Could not export. Browser storage could not be opened, so no file was written."** — in French, **"Export impossible. Le stockage du navigateur n’a pas pu être ouvert, aucun fichier n’a donc été écrit."**. Switch the language in Settings and repeat once to see both. The page must stay usable: no blank screen, no stuck spinner, the Export button still clickable.
- [ ] **Nothing was written.** No file may land in your downloads folder for that press — check the browser's download list, not just the folder. The message says "no file was written" and it must be literally true; a zero-byte or truncated `Gones Archive.json` would be a defect.
- [ ] **The real reason is still in the console — the friendly message did not swallow it.** With DevTools → Console open, repeat the failing press. Alongside the banner you must see the structured boundary line `{"level":"error","boundary":"app-header.fullDataExport","context":{},"message":"indexedDbUnavailable"}` (Chromium's blocked-storage variant reads `indexedDbOpenFailed`, and a version-change lock held by another tab reads `indexedDbBlocked`). The point of this slice is that the user gets one plain sentence while the operator keeps the exact cause. Confirm too that **no unhandled promise rejection** is logged any more — that red "Uncaught (in promise)" entry was the old behaviour and must be gone.
- [ ] **The banner clears on the next success — it does not stick.** Without reloading, undo the block (Firefox: set `dom.indexedDB.enabled` back to `true`; Chromium: re-allow site data), reload the tab, and press **Export** again. The banner must be **gone** the moment the export succeeds, and the file must download normally. A failure banner that survives a later successful export is a defect.
- [ ] **The success path is byte-for-byte what it was.** Diff the file from the previous step against the baseline file from step one: `diff <(jq -S 'del(.checksum)' first.json) <(jq -S 'del(.checksum)' second.json)` must print nothing, and both checksums must verify (re-import each file through **Import league(s)** and confirm no error banner). This commit only wrapped the two existing statements in a `try` — nothing about what gets written changed.
- [ ] **Signed in, the refusal is unchanged.** Sign in as any account and press **Export** on `/archive/league-seasons`. The banner must still read the *server-unavailable* refusal ("Server Leagues could not be loaded, so no file was written…"), not the new storage message, and DevTools → Network must show no archive read at all: that guard runs before the `try` and this commit did not move it.
- [ ] **Known residual — one message covers every store failure.** `indexedDbUnavailable` (API missing), `indexedDbOpenFailed` (open refused) and `indexedDbBlocked` (another tab holds a version-change lock) all render the same sentence; only the console line distinguishes them. Per-cause copy was explicitly out of scope for this slice (the finding is severity low). Likewise there is no retry affordance — the user's recourse is to unblock storage and press Export again, which the step above proves works.
- [ ] **Known residual — the export button is the only handler this slice covers.** Other shell handlers already had their own `catch`; nothing else in `app.component.ts` changed. If you find a different silent button, it is a separate finding, not a regression from this commit.
- [ ] **The commit is the three files the ticket names plus this checklist.** `git show --stat HEAD` → `src/app/app.component.ts`, `src/app/app.component.export.test.ts`, `src/app/i18n/messages.ts`, `artifacts/manual_test_checklist.md`. No backend, no Cypress, no template change: `git show HEAD -- backend/ cypress/ ops/` must print nothing, and the only `src/app/i18n/` change must be the one new key added to **both** catalogs (`git show HEAD -- src/app/i18n/messages.ts | grep -c '^+.*fullDataExportFailed'` → `2`). `npm run e2e:ci` and `npm run cy:run` were deliberately not run for this slice (their teardown destroys local database volumes); no Cypress spec exercises the failure — `cypress/e2e/power-user-gating.cy.js` only asserts the button is visible.

## T28 archive-cache-retry-open

This commit changes one line of memoization in `ArchiveCacheService.database()` (`src/app/backend/archive-cache.service.ts`). Before it, the service held its IndexedDB handle with `this.handle ??= openDatabase(...)`. A rejected promise is a settled value, so `??=` never replaced it: **one** failed open — storage disabled, a quota refusal, a version-change lock held by another tab — poisoned the handle for the entire life of the tab. Every archive navigation afterwards refetched the full catalogs from the server and `clearAll()` became a no-op, silently, because all four call sites (`readOne`, `writeOne`, `readAllYearPartitions`, `clearAll`) swallow a cache failure as a miss by design. From this commit a rejected open clears the handle, so the *next* call opens again. What needs your eyes is the word **next**: recovery must happen without reloading the tab, because a reload also builds a fresh service and would hide the bug entirely. The happy path must be untouched — a warm cache must still serve from IndexedDB with no network request.

Work **signed out** on the server-backed archive (the cached catalogs are the public anonymous server answers; no account is needed). Run from the repository root with `npm run dev` (API on `http://127.0.0.1:5080`, app on `http://127.0.0.1:4200`). Do **not** run `docker compose down --volumes` or `npm run db:reset` at any point. Keep DevTools open on **Network** and **Application → Storage → IndexedDB** throughout; the whole slice is invisible without them.

- [ ] **Baseline — prove the cache is doing its job before you break anything.** Signed out, open `http://127.0.0.1:4200/archive/league-seasons`. In DevTools → Application → IndexedDB a database named `gones-archive-cache` must appear with exactly four stores: `leagues`, `league-seasons`, `year-partitions`, `meta`. Open `leagues` — it must hold one row under the key `catalog` carrying `items`, `totalCount`, `truncated` and an ISO `fetchedAt`. On the Network tab, filter on `all` and confirm the first visit issued `GET /api/archive/leagues/all` and `GET /api/archive/league-seasons/all`.
- [ ] **Baseline, second half — a warm cache serves without the network.** Without reloading, navigate to `/archive/tournaments` and back to `/archive/league-seasons` a couple of times. Those two `…/all` requests must **not** fire again: the record is fresh for 24 hours and the repository returns it before it ever reaches the client. If you do see a refetch on every navigation, your cache is already cold — delete the `gones-archive-cache` database, reload once, and start this section over.
- [ ] **Arm a transient open failure — in the page, without reloading.** This is the only simulation that proves the fix, because the service is a root singleton: a reload discards the poisoned handle and would make the old code look correct. In the DevTools **Console**, on the archive tab, paste:

    ```js
    window.__realOpen = indexedDB.open.bind(indexedDB);
    indexedDB.open = function () {
      const request = { error: new DOMException('Simulated open failure', 'UnknownError'), onerror: null, onsuccess: null, onupgradeneeded: null, onblocked: null };
      setTimeout(() => request.onerror && request.onerror(), 0);
      return request;
    };
    ```

    That reproduces exactly what `openDatabase` (`src/app/backend/indexed-db.ts`) sees on a real failure: a request that fires `onerror` carrying a `DOMException`. Then delete the `gones-archive-cache` database in Application → IndexedDB so the next navigation must go through an open.
- [ ] **The failure is silent and harmless — that part must not change.** Still with the stub armed, navigate between `/archive/league-seasons` and `/archive/tournaments`. Both pages must render their rows normally: **no** error banner, **no** blank screen, **no** stuck spinner, and no red `Uncaught (in promise)` entry in the console. On Network you must now see `GET /api/archive/leagues/all` and `GET /api/archive/league-seasons/all` fire again on **every** navigation — the cache is a miss, so the server answers each time. A cache that cannot be opened is a miss, never a page error; that is the service's documented contract and this commit does not touch it.
- [ ] **Heal the store and confirm the very next navigation recovers — the point of the whole commit.** *Without reloading the tab*, paste in the Console:

    ```js
    indexedDB.open = window.__realOpen;
    ```

    Now navigate once, from `/archive/league-seasons` to `/archive/tournaments` (or back). In Application → IndexedDB the `gones-archive-cache` database must **reappear**, with all four stores and a fresh `catalog` row under `leagues`. Navigate once more: the two `…/all` requests must stop firing again. That is the fix — the handle was retried on the next call, in the same page session, with no reload.
- [ ] **The regression, for contrast — confirm you would have seen the bug.** Optional but worth one minute: `git stash` this commit (or check out `76b088b`), repeat the three steps above, and observe that after healing `indexedDB.open` the database never comes back and every navigation keeps hitting the server, for as long as the tab lives. Only a full page reload restores caching. Restore your working tree afterwards (`git stash pop` / `git checkout main`).
- [ ] **A persistent failure stays bounded — no retry storm.** Re-arm the stub from the earlier step and leave it armed. Navigate back and forth between the two archive routes six or eight times. Each navigation must produce **at most one** additional `open` attempt per cache operation — the retry is demand-driven, one attempt per call, never a loop, never exponential. The console must stay free of unhandled promise rejections, and the CPU must stay idle between navigations. If you see the tab spinning or the console filling, that is a defect.
- [ ] **Concurrent readers still share one open.** Clear the stub and the database, then reload the tab and land directly on `/archive/league-seasons`, which loads several catalogs at once. In Application → IndexedDB the database must be created exactly once, and the page must render one consistent set of rows. The success path memoizes as it always did: parallel callers during one pending open share that open, and a fulfilled handle is reused for the life of the tab.
- [ ] **Browser-native variant, as a second opinion.** For a non-synthetic failure: Firefox `about:config` → set `dom.indexedDB.enabled` to `false`, reload, browse the archive (everything works, nothing cached); Chromium `chrome://settings/content/siteData` → **"Don't allow sites to save data on your device"**, reload, same result. Note that both of these require a reload to take effect and to undo, so they demonstrate the *failure* is harmless but **not** the same-session recovery — the Console stub above is the only way to see that. Undo the browser setting when you are done.
- [ ] **The archive itself is unchanged.** With caching healthy, confirm the pages still behave exactly as before: `/archive/league-seasons` lists its seasons, `/archive/tournaments` lists its tournaments, opening a tournament detail and its result page works, and a browser-local row (power-user setting on, signed out) still shows alongside the server rows. This slice touched cache-handle lifetime only — no read model, no write path, no UI.
- [ ] **Known residual — the failure is still invisible to the user.** There is no banner, no toast and no console line when the cache cannot be opened; the only symptom is that the archive gets its data from the server every time. Surfacing that was explicitly out of scope for this slice (the finding is severity low, and the swallow is the documented cache-miss contract). If you want to know whether your cache is alive, look for the `gones-archive-cache` database in DevTools.
- [ ] **Known residual — no `onversionchange` handling.** The sibling `ServerReadCacheService` closes and drops its handle when another tab requests a version change; the archive cache never had that and this commit did not add it. So an upgrade held open by a second tab can still block this one until the tab is closed — a different behaviour change, deliberately not made here.
- [ ] **The commit is the two files the ticket names plus this checklist.** `git show --stat HEAD` → `src/app/backend/archive-cache.service.ts`, `src/app/backend/archive-cache.service.test.ts`, `artifacts/manual_test_checklist.md`. Nothing else moved: `git show HEAD -- backend/ cypress/ ops/ src/app/i18n/ src/app/backend/indexed-db.ts src/app/backend/server-read-cache.service.ts src/app/backend/in-memory-indexeddb.fake.ts` must print nothing, and the production diff must be a single hunk inside `database()` (`git show HEAD -- src/app/backend/archive-cache.service.ts | grep -c '^@@'` → `1`). `npm run e2e:ci` and `npm run cy:run` were deliberately not run for this slice (their teardown destroys local database volumes); no Cypress spec exercises the archive cache handle.

## T29 logout-lock-bound

- [ ] **Baseline — a normal logout still purges the cache instantly.** Start the app (`npm run dev`), sign in at `http://127.0.0.1:4200`, and browse a couple of signed-in pages so the private cache fills. In DevTools → Application → IndexedDB a database named `gones-cache` must appear with one store, `reads`, holding rows keyed `<yourUserId>:<resource>`. Now sign out. The logout must complete immediately (well under a second), the `gones-cache` database must vanish from the IndexedDB list, and `localStorage` must **not** contain the key `gones.auth.privatePurgeRequired`. That is the unchanged happy path; this commit must not slow it down or change it in any way.
- [ ] **Arm the blocked delete — a second tab that holds `gones-cache` open and never lets go.** This is the whole point of the commit, and a plain second app tab will *not* reproduce it: the app's own store sets `database.onversionchange = () => database.close()` (`open()` in `src/app/backend/server-read-cache.service.ts`), so a real sibling tab closes on request and the delete goes through. You need a peer that holds a raw connection with no `versionchange` handler — a frozen tab, a back-forward-cached tab, or the stub below. Sign in again in tab 1 and browse enough to recreate `gones-cache`. Then open a **second tab** on the same origin (`http://127.0.0.1:4200`), and in **that tab's** DevTools Console paste:

    ```js
    window.__peer = await new Promise((resolve, reject) => {
      const request = indexedDB.open('gones-cache');
      request.onsuccess = () => resolve(request.result);
      request.onerror = () => reject(request.error);
    });
    // Deliberately NO onversionchange handler and no close(): this is the frozen peer.
    console.log('peer holding gones-cache open', window.__peer.name, window.__peer.version);
    ```

    You must see the `peer holding gones-cache open gones-cache 1` line. Leave this tab open and untouched for the next step — do not sign in or out in it, because an auth transition there would queue on the same origin-wide lock you are about to test.
- [ ] **Reproduce and time it — logout settles instead of hanging.** Back in **tab 1**, open the Console, then sign out and **start a stopwatch**. Expected observation: logout does *not* return instantly this time — it takes about **10 seconds** — and then it **completes**: the app lands on the signed-out view exactly as a normal logout does, with no error banner, no error toast, no blank screen and no stuck spinner. Ten seconds of delay, then a clean signed-out state. What must **not** happen is the pre-fix behaviour: a logout that never finishes, a button that stays busy forever, and a session that never actually ends.
- [ ] **Confirm the failure was loud, not swallowed.** In tab 1's Console, after the logout settles, you must see two structured error lines, in this order:

    ```
    {"level":"error","boundary":"server-read-cache.purge","context":{},"message":"indexedDbDeleteBlocked"}
    {"level":"error","boundary":"auth.session-purge","context":{},"message":"indexedDbDeleteBlocked"}
    ```

    The exact message string must be `indexedDbDeleteBlocked`. If you instead see `indexedDbDeleteFailed`, or no line at all, the deadline is not the thing that fired and the run is not a valid reproduction.
- [ ] **Confirm the purge is still owed — the retry must survive.** Still in tab 1, in Application → Local Storage, the key `gones.auth.privatePurgeRequired` must still be present with value `1`, and in Application → IndexedDB the `gones-cache` database must **still be listed** (the delete never completed — the browser has it queued behind the frozen peer). This is the intended trade: the wait is bounded, the delete is not abandoned. A missing marker here would be the real defect, because it would mean the app believes it purged data it did not purge.
- [ ] **Confirm the origin-wide lock was released — no tab is wedged.** Immediately after the logout settles in tab 1, sign in again **in tab 1**, and then sign in/out once **in the second tab** (the frozen peer tab). Both auth transitions must proceed normally and promptly. Before this commit, the pending delete pinned `navigator.locks` name `gones.auth.session-transition` for the whole origin, so *every* subsequent sign-in or sign-out in *every* tab queued behind it forever. If any auth action in any tab now hangs, the lock is not being released and this commit has failed its purpose.
- [ ] **Confirm the retry actually happens on the next establishment.** With the marker still set from the step above, release the peer: in the **second tab's** Console run `window.__peer.close()`. Then in tab 1 sign in again (or reload and sign in). Once signed in, check Application → Local Storage: `gones.auth.privatePurgeRequired` must now be **gone**, and the `gones-cache` database must have been dropped and recreated fresh (it must contain no rows keyed with the *previous* session's user id). That closes the loop the ticket promises: the deadline rejects, the marker stays, and the next coordinated establishment finishes the purge.
- [ ] **A block that lifts on its own must not be penalised.** Re-arm the frozen peer in the second tab, sign in and browse in tab 1 so `gones-cache` refills, then sign out in tab 1 **and within two or three seconds** run `window.__peer.close()` in the second tab. Logout must complete promptly at that moment — **not** wait out the remaining ten seconds — `gones-cache` must disappear, and `gones.auth.privatePurgeRequired` must be cleared. The deadline is a ceiling on the wait, never a fixed delay, and a briefly blocked delete finishes normally with no late error appearing in the console afterwards.
- [ ] **Account deletion is bounded by the same fix.** The same teardown runs for account deletion, so with the frozen peer armed, delete a throwaway account from the account settings page. It must settle (within about ten seconds if blocked) and leave the app in a signed-out state rather than hanging. Only do this with a disposable account — account deletion is irreversible.
- [ ] **Known residual — the ten-second wait is not surfaced in the UI.** There is no spinner label, toast or copy telling the user why this particular logout took ten seconds; it simply takes ten seconds and then works. Surfacing that was out of scope for this slice (severity low, no UI change permitted by the ticket). Likewise, the ceiling bounds only the database delete: any *other* reset registered with `SessionScopeService` — including the Cache Storage purge in `purgeCachedApiResponses()` — is still unbounded, and `src/app/auth/session-scope.service.ts` was explicitly fenced off from this commit.
- [ ] **The commit is the two files the ticket names plus this checklist.** `git show --stat HEAD` → `src/app/backend/server-read-cache.service.ts`, `src/app/backend/server-read-cache.service.test.ts`, `artifacts/manual_test_checklist.md`. Nothing else moved: `git show HEAD -- backend/ cypress/ ops/ src/app/i18n/ src/app/auth/ src/app/backend/indexed-db.ts src/app/backend/archive-cache.service.ts src/app/backend/in-memory-indexeddb.fake.ts` must print nothing, and the production diff must stay outside the store class (`git show HEAD -- src/app/backend/server-read-cache.service.ts | grep -c '^@@'` → `2`, both hunks in the module-level `deleteDatabase` region). `npm run e2e:ci` and `npm run cy:run` were deliberately not run for this slice (plan Scope Out; their teardown destroys local database volumes); no Cypress spec exercises a blocked database delete.

## T30 bootstrap-generation-race

- [ ] **Read this first — what this commit does and does not claim.** This slice makes the refresh-cookie spend, the session-generation advance and the access-token publication happen inside **one** origin-wide auth-lock hold, instead of spending the cookie under one hold and advancing the generation under a later, separate one. That is the whole behavioural claim. It does **not** make two simultaneously restoring tabs both end up signed in: the session generation is an origin-wide counter that every successful establishment advances, so two concurrent restores still leave one tab superseded. What changed for that superseded tab is that it now ends **cleanly signed out with no access token retained**, where before this commit it could report a failed startup while still holding a live bearer token in memory. The remaining race (F1 residual) is knowingly still open and tracked as its own follow-up; do not record this checklist as proof that it is fixed.
- [ ] **Baseline — a single-tab reload restore is completely unchanged.** Start the app (`npm run dev`), sign in at `http://127.0.0.1:4200`, and browse a couple of signed-in pages. Now reload the page (F5) with only this one tab open. You must land back on the same signed-in view: your username/avatar in the chrome, no flash of the signed-out state beyond the normal startup render, no sign-in redirect, no error banner and no error toast. In DevTools → Network, filter on `refresh` — there must be exactly **one** `POST` to the refresh endpoint for the reload, returning `200`, followed by one `GET` of the profile (`me`) endpoint. This is the path 99% of users take and it must look exactly as it did before the commit.
- [ ] **The generation advances exactly once per restore.** Still in the single tab, open DevTools → Application → Local Storage for `http://127.0.0.1:4200` and note the value of the key `gones.auth.sessionGeneration` — it is a plain integer. Reload the page once and read it again. It must have gone up by **exactly one**, not two. Reload twice more and confirm it goes up by exactly one each time. A jump of two on a single reload would mean the generation is being advanced in two places and the atomicity this commit is about has been lost.
- [ ] **Sign-in and sign-out still work end to end.** Sign out, and confirm you land on the signed-out view with no error. Sign in again with the same account and confirm the signed-in view returns with your profile loaded. If the project's OAuth provider is configured locally, complete one OAuth sign-in as well. These three paths (`login`, `completeOAuth`, `verifyOAuthEmail`) share the establishment code this commit restructured, so all three must behave exactly as they did before — same screens, same timing, no new error.
- [ ] **The two-tab simultaneous restore — set it up.** This is the case the commit is really about, and you have to force the overlap deliberately, because two hand-timed reloads usually do not overlap. Sign in, then open a **second tab** on `http://127.0.0.1:4200` so both tabs are signed in as the same account. In **both** tabs open DevTools → Network and set throttling to **Slow 3G** (this widens the refresh round trip to hundreds of milliseconds, which is what makes the overlap reproducible). Now reload **both** tabs as close together as you can: click into tab 1, press F5, immediately switch to tab 2 and press F5 — well inside one second. In each tab's Network panel you should see the two refresh round trips overlapping in the waterfall rather than running one after the other; if tab 2's refresh only starts after tab 1's has finished and tab 1 is already rendered, the tabs did not actually race, so try again (throttle harder or use two reloads triggered from two windows side by side).
- [ ] **The two-tab simultaneous restore — what each tab must show.** Expected, and this is the honest expectation, not a workaround: **one** tab comes back **signed in** — profile loaded, signed-in chrome, normal view. The **other** tab comes back **signed out** — it shows the ordinary signed-out/sign-in view. Which of the two loses is decided by which one publishes its session last and is not worth predicting. What must **not** happen in either tab: an error banner, an error toast, a blank screen, a stuck spinner, an infinite redirect loop, or a tab wedged mid-load. The superseded tab must look like a plain signed-out app, and a single reload of that tab (with the other tab now idle) must sign it straight back in. Both refresh calls must have returned `200`: check the Network panel in **both** tabs — neither tab may show a `401`/`403` on its refresh, because a rejected round trip would mean the two tabs replayed one cookie, which is a different and much more serious failure than the one described here.
- [ ] **The superseded tab must carry no access token.** In whichever tab came back **signed out** in the step above, and without reloading it, try to reach a signed-in-only page by typing its URL directly (for example the account settings page). You must be redirected to sign-in rather than shown the page. Then, still in that tab, open DevTools → Network, clear it, and click around the app to make it issue at least one API request. Inspect the request headers of those calls: **none of them may carry an `Authorization: Bearer …` header**. That absence is the actual fix in this commit — before it, that tab could report a failed startup while still holding the token it had published, so its requests would have gone out authenticated even though the UI said signed out. If you see a `Bearer` header from the signed-out tab, this commit has regressed and that is a blocker.
- [ ] **The winning tab is untouched by the other tab's failure.** In the tab that came back **signed in**, confirm it stays fully usable while the other tab sits on its signed-out view: navigate between two or three signed-in pages, and confirm requests still carry `Authorization: Bearer …` and return `200`. The superseded tab's cleanup must be local to that tab — it must never invalidate the session the winning tab legitimately owns. If the signed-in tab gets kicked to sign-in when the other tab finishes failing, that is a blocker.
- [ ] **A quiet second tab must not be disturbed at all.** Sign in on both tabs again, leave tab 2 idle on a signed-in page, and reload only **tab 1**. Tab 2 must not change: it must not be signed out, redirected, blanked or reloaded, and it must still make authenticated requests afterwards (click a link in tab 2 and confirm a `200` with a `Bearer` header). A single tab restoring is expected to leave every other tab alone.
- [ ] **Logout is unchanged, including the bounded-teardown behaviour from the previous slice.** Sign in, then sign out normally in a single tab: it must complete immediately, land on the signed-out view, and drop the `gones-cache` IndexedDB database as before. This commit did not touch `logout()`, `runTeardown()`, `deleteAccount()` or `clear()`, so if any of the logout behaviour from the previous slice's checklist has changed, something outside this ticket's fence moved.
- [ ] **Known residual — the two-tab race itself is still open.** To be explicit, because the checklist above will make it look like a bug if you do not know it is intended: two tabs that genuinely restore at the same moment still end with exactly one of them signed out, and that tab's user has to reload to get back in. This commit narrows the damage (the generation advance can no longer be interleaved with another tab's cookie spend, and the loser no longer keeps a stale token) but does not eliminate the race. Eliminating it requires the origin-wide coordination layer to record which profile owns each generation, so a tab superseded by a peer establishing the *same* profile can re-anchor instead of failing; that cannot be done safely inside `auth.service.ts` alone, because a tab only knows its own profile and could otherwise bind one user's profile into another user's cache scope. That work is tracked separately and is deliberately not in this commit.
- [ ] **The commit is the two files the ticket names plus this checklist.** `git show --stat HEAD` → `src/app/auth/auth.service.ts`, `src/app/auth/auth.service.bootstrap.test.ts`, `artifacts/manual_test_checklist.md`. Nothing else moved: `git show HEAD -- backend/ cypress/ ops/ src/app/i18n/ src/app/backend/ src/app/api/ src/app/auth/auth-session-coordination.service.ts src/app/auth/session-scope.service.ts src/app/auth/session-catalog-sync.service.ts src/app/auth/fake-web-locks.ts` must print nothing. `npm run e2e:ci` and `npm run cy:run` were deliberately not run for this slice (plan Scope Out; their teardown destroys local database volumes); no Cypress spec drives two tabs restoring at the same instant.

## T31 remove-legacy-redirect

- [ ] **Read this first — what this commit does and does not claim.** This slice deletes one dead static file (`src/legacy-pages/leagues.html`), the `angular.json` assets rule that copied it into every build as `pages/leagues.html`, and one stale sentence in `docs/leagues-page-layout.md` that claimed the prototype still lived in the repo. That is the whole change: no route was added, removed or reordered, no application code was touched, no API changed, and no persisted data moved. The deleted file was a two-hop dead end — it redirected to `../leagues`, and `/leagues` is itself not a declared route, so the redirect landed on the application's own 404 page. It was only ever reachable on the GitHub Pages host that T13 retired. Nothing a user does today should look different except as described in the next step.
- [ ] **What a visitor now sees at the removed URL — read the expected result carefully, it is not an HTTP 404.** Bring up the release image with `docker compose --profile release up --build -d` (the SPA answers on `http://127.0.0.1:8081`) and open `http://127.0.0.1:8081/pages/leagues.html`. Expected: **the Gones 404 page, rendered by the application, reached in one hop with no redirect.** In DevTools → Network, the document request for `/pages/leagues.html` returns **HTTP 200**, not 404, and its response body is `index.html`. That is correct and intended: `deploy/nginx/default.conf.template` serves everything through `location / { try_files $uri $uri/ /index.html; }` and has no `/pages` exception, so a missing file falls back to the single-page-application entry point, the router then matches its catch-all and renders `NotFoundComponent`. What must **not** appear: the old French redirect page titled "Redirection vers Gones", any `<meta http-equiv="refresh">`, any automatic navigation to `/leagues`, or a second document request in the waterfall. Before this commit that URL served the real redirect file and then bounced to `/leagues`, which landed on the same 404 page — two hops to the same destination. One hop now.
- [ ] **Note the discrepancy with the ticket's own wording, so it is not later filed as a bug.** The ticket's `## Outputs` section says the URL "now 404s directly at the web-server level". It does not, and cannot, with the nginx configuration this repository actually ships: the SPA fallback above returns 200 with the application 404 page. The user-visible outcome (the 404 page) and the "one dead hop instead of two" claim are both correct; only the status-code framing in that sentence is wrong. No configuration change was made to bring the behaviour in line with that sentence — that would have meant editing nginx, which this ticket explicitly puts out of scope.
- [ ] **Same check on the development server.** Run `npm run dev` and open `http://127.0.0.1:4200/pages/leagues.html`. The Angular development server applies the same single-page-application fallback, so you must again get the Gones 404 page in one hop, with no redirect and no "Redirection vers Gones" document.
- [ ] **The old redirect target is unchanged and still dead.** Open `http://127.0.0.1:8081/leagues` (and `/leagues-archive`). Both must still render the Gones 404 page exactly as they did before this commit — this slice deliberately did not add, alias or restore any of those routes. The live archive lives at `/archive/league-seasons`, `/archive/league-seasons/:seasonId` and `/archive/tournaments/:tournamentId`; confirm those three still load normally.
- [ ] **No stale copy can be served from the service worker.** On a browser that already had the site installed before this change, hard-reload `http://127.0.0.1:8081` and then request `/pages/leagues.html` again. You must still get the 404 page, never a cached redirect. The reason it is safe: `ngsw-config.json` caches only `/favicon.ico`, `/index.html`, `/manifest.webmanifest`, `/*.css`, `/*.js` and `/assets/**`, so `/pages/**` was never in any asset group and never entered the service-worker cache; its `navigationUrls` also excludes `!/**/*.*`, so a dotted path like this one is fetched from the network rather than answered from the shell.
- [ ] **The two surviving build assets must still be there — this is the regression guard for the `angular.json` edit.** The assets array went from three entries to two, so confirm nothing else was dropped with it. In the running app, images and icons must load (no broken image placeholders anywhere on the home page), `http://127.0.0.1:8081/manifest.webmanifest` must return the web app manifest, and the browser must still offer to install the application. Equivalently against a fresh build: `npm run build` then confirm `dist/gones/browser/assets/` exists and is populated and `dist/gones/browser/manifest.webmanifest` exists.
- [ ] **The build emits no `pages/` directory at all.** Run `npm run build`, then `find dist -name 'leagues.html' -o -type d -name pages` — it must print nothing. Note that a stale `dist/` from before this commit will contain `dist/gones/browser/pages/leagues.html`; Angular deletes the output path on each build, so run the build first and do not judge this from an old tree.
- [ ] **The documentation no longer points at a file that does not exist.** Open `docs/leagues-page-layout.md` and read the route note at the top: it must say the `leagues.html` references below are the "historical static prototype (since removed from the repo)", with no mention of `src/legacy-pages/`. The later mentions of `leagues.html` in that document's body (in the layout prose) are intentionally left alone as historical wording, as is `docs/mvp-from-scratch-plan.md` and the superseded `docs/adr/0013-host-angular-spa-on-github-pages.md`, which still records the retired Pages URL as part of the decision history. Those three are records of what was, not instructions for what is.
- [ ] **The commit is the three files the ticket names plus this checklist.** `git show --stat HEAD` → `angular.json`, `docs/leagues-page-layout.md`, `src/legacy-pages/leagues.html` (deleted), `artifacts/manual_test_checklist.md`. Nothing else moved: `git show HEAD -- backend/ cypress/ ops/ src/app/ deploy/ ngsw-config.json package.json .github/` must print nothing, and `git show HEAD --stat -- src/` must show only the deletion. `npm run e2e:ci` and `npm run cy:run` were deliberately not run for this slice (plan Scope Out; their teardown destroys local database volumes); no Cypress spec ever requested `/pages/leagues.html`.
- [ ] **When you are done with the release container, stop it without destroying data.** Use `docker compose --profile release down` on its own. Do not add `--volumes` — that wipes the local database volumes and forces a full reseed.

## T32 health-security-headers

- [ ] **Read this first — what this commit does and does not claim.** This slice changes one production file, `deploy/nginx/default.conf.template`, and adds one static ops test, `ops/nginx-security-headers.test.ts`. Inside the template it restates the eight server-level security headers verbatim inside `location = /runtime-config.json` and `location = /health`, and replaces `add_header Content-Type text/plain;` in the health block with `default_type text/plain;`. Nothing else moved: the server-level header values are byte-identical, `location /` was not touched, and no entrypoint, `Dockerfile`, `compose.yaml` or application code changed. The reason the restatement is necessary rather than redundant: nginx's `add_header` inheritance is **level-scoped** — a `location` that defines *any* `add_header` inherits **none** from the enclosing `server`. Both of these locations already defined one, so both were silently shipping without the other seven. Everything below is a check on the running release image; the automated gate can only read the template.
- [ ] **Bring the release image up.** Run `GONES_API_BASE_URL=http://127.0.0.1:5080 GONES_AUTH_V1=true GONES_ADMIN_V1=true docker compose --profile release up -d --build --wait frontend-release`. This also starts `postgres`, `migrator` and `api`, because `frontend-release` waits on a healthy API. The static site answers on `http://127.0.0.1:8081` (the container listens on `8080`; `compose.yaml:174` maps `127.0.0.1:8081:8080`). Do not tear this down with `--volumes` — see the last step.
- [ ] **The container actually started, which is itself the nginx syntax check.** `docker compose --profile release ps frontend-release` shows the container `running` (and healthy). nginx refuses to start on a malformed configuration, so a running container proves the rendered `/tmp/gones-nginx/default.conf` parsed. If you want it explicitly: `docker compose --profile release exec frontend-release nginx -t` must print `syntax is ok` and `test is successful`.
- [ ] **`/health` emits exactly ONE `Content-Type` header — count it on the raw wire, not in a parsed header map.** Run this exact command:

  ```
  curl -sS -D - -o /dev/null http://127.0.0.1:8081/health | grep -ci '^content-type:'
  ```

  It must print exactly `1`. **Read this carefully: the raw count is the whole point of this check.** Before this commit the health block used `add_header Content-Type text/plain;`, and nginx's `add_header` *appends* a field to the response header list — it does not replace one — so the response carried a second `Content-Type` on top of the one the content type had already been resolved to. Almost every convenient way of looking at headers hides that: DevTools, `fetch()`/`response.headers.get()`, Postman, and most HTTP client libraries all collapse repeated fields into a single map entry, so a duplicate looks identical to a correct single header. `grep -c` over `curl -D -` output counts the actual lines nginx put on the wire, which is the only view that can tell the two apart. If this prints `2`, the fix has regressed.
- [ ] **…and its value is `text/plain`, with the body unchanged.** `curl -sSi http://127.0.0.1:8081/health` shows `HTTP/1.1 200 OK`, a single `Content-Type: text/plain` line (a `; charset=…` suffix is acceptable), and the body `live`. The status and body must be exactly as before this commit — only the typing mechanism changed, from an appended header to `default_type`.
- [ ] **`/health` now carries all eight security headers, each exactly once.** Run:

  ```
  curl -sS -D - -o /dev/null http://127.0.0.1:8081/health \
    | grep -iE '^(x-content-type-options|x-frame-options|referrer-policy|cross-origin-opener-policy|cross-origin-resource-policy|permissions-policy|strict-transport-security|content-security-policy):'
  ```

  Exactly eight lines must come back — one each for `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy: no-referrer`, `Cross-Origin-Opener-Policy: same-origin`, `Cross-Origin-Resource-Policy: same-origin`, `Permissions-Policy: …`, `Strict-Transport-Security: max-age=63072000; includeSubDomains; preload` and `Content-Security-Policy: …`. Before this commit **none** of them appeared on this endpoint. Pipe through `| wc -l` if you want the count rather than the lines; it must be `8`, not more (a repeated field would mean the header is being set at two levels). Note that `Strict-Transport-Security` does appear here even though you are on plain HTTP — the directive carries `always`, so nginx emits it regardless; browsers simply ignore HSTS on a non-HTTPS response. That is expected, not a finding.
- [ ] **`/runtime-config.json` carries the same eight, plus `Cache-Control: no-store`, each exactly once.** Run the same `grep -iE` command against `http://127.0.0.1:8081/runtime-config.json` — again exactly eight lines — and then `curl -sS -D - -o /dev/null http://127.0.0.1:8081/runtime-config.json | grep -ci '^cache-control: *no-store'`, which must print `1`. Also confirm with `curl -sSi http://127.0.0.1:8081/runtime-config.json` that the response is still `200`, still `Content-Type: application/json`, and still the declaration document the app reads (`{"dataMode":"server","apiBaseUrl":"http://127.0.0.1:5080","features":{"authV1":true,"adminV1":true}}`). Before this commit only `Cache-Control` and `X-Content-Type-Options` appeared here; the other seven were missing.
- [ ] **The Content-Security-Policy on both endpoints has the real origin substituted, not the literal placeholder.** In the `Content-Security-Policy` line from either endpoint above, the `connect-src` directive must read `connect-src 'self' http://127.0.0.1:5080` (or whatever `GONES_API_BASE_URL` you started the container with). It must **not** contain the literal text `${GONES_API_ORIGIN}`. This is the one genuinely new risk in the change: the two restated CSP lines each carry that placeholder, and they are only correct because `deploy/nginx/gones-runtime-entrypoint.sh:35` renders the whole template through `envsubst '${GONES_API_ORIGIN}'`, which substitutes every occurrence rather than just the first. If you see the raw `${GONES_API_ORIGIN}` text in a response header, the restated lines are not being rendered and the CSP is broken.
- [ ] **Nothing regressed on the endpoints that already had these headers.** The application shell and static assets were served through `location / { try_files … }`, which defines no `add_header` of its own and therefore inherits the full set from the server level — this commit did not touch that block, and the check is that it still behaves that way. Run the same eight-header `grep -iE` against `http://127.0.0.1:8081/` (the SPA document) and against one hashed asset (pick a `.js` or `.css` URL out of the page source, e.g. `http://127.0.0.1:8081/main-<hash>.js`). Both must return exactly eight lines with the same values as before, and each header exactly once — in particular the document must **not** show a doubled `Content-Security-Policy` or `X-Content-Type-Options`, which is what you would see if the restated directives had accidentally been added at server level as well as in a location.
- [ ] **The app still boots against the stricter responses.** Open `http://127.0.0.1:8081` in a browser and confirm the site loads normally and navigates. Then in DevTools → Console, confirm there is **no** CSP violation report, and in DevTools → Network confirm the `runtime-config.json` request returns `200` and the app picked up `dataMode: server` (it reads that document before it bootstraps, at `src/main.ts:44`). The new `Cross-Origin-Resource-Policy: same-origin` on `runtime-config.json` is safe precisely because the app fetches it same-origin via `new URL('runtime-config.json', document.baseURI)`; if you ever serve the SPA from a different origin than that document, this header would start blocking it.
- [ ] **The Docker healthcheck is unaffected.** `docker inspect --format '{{.State.Health.Status}}' $(docker compose --profile release ps -q frontend-release)` reports `healthy`. `Dockerfile:77` probes with `wget -q --spider http://127.0.0.1:8080/health`, which only looks at the status line, so neither the added headers nor the content-type change can affect it. Worth confirming anyway, because `/health` has `access_log off;` — deliberately kept, since probes every 10s would flood the access log — so a broken health endpoint leaves no log trail and this is the only place it would surface.
- [ ] **The commit is the two files the ticket names plus this checklist.** `git show --stat HEAD` → `deploy/nginx/default.conf.template`, `ops/nginx-security-headers.test.ts`, `artifacts/manual_test_checklist.md`. Nothing else moved: `git show HEAD -- backend/ cypress/ src/ angular.json package.json compose.yaml Dockerfile deploy/nginx/gones-runtime-entrypoint.sh deploy/nginx/gones-data-authority.sh deploy/nginx/runtime-include.conf` must print nothing. Confirm the server-level block is untouched: `git show HEAD -- deploy/nginx/default.conf.template | grep '^-'` must show exactly one removed line, `add_header Content-Type text/plain;`. `npm run e2e:ci` and `npm run cy:run` were deliberately not run for this slice (plan Scope Out; their teardown destroys local database volumes); no Cypress spec asserts on response headers.
- [ ] **When you are done with the release container, stop it without destroying data.** Use `docker compose --profile release down` on its own. Do not add `--volumes` — that wipes the local database volumes and forces a full reseed.

## T33 manifest-icon-sizes

- [ ] **Read this first — what this commit does and does not claim.** This slice changes metadata only. `src/manifest.webmanifest` previously listed `assets/gones_logo.png` twice, at `"sizes": "192x192"` and `"sizes": "512x512"`, both with `"purpose": "any maskable"`. The raster is neither of those sizes and never was: its PNG IHDR header says **1916x484** (verify yourself with `node -e "const b=require('node:fs').readFileSync('src/assets/gones_logo.png');console.log(b.readUInt32BE(16)+'x'+b.readUInt32BE(20))"`, or `identify src/assets/gones_logo.png` if you have ImageMagick). The manifest now carries **one** entry, `"sizes": "1916x484"`, `"purpose": "any"`. No image file was created, cropped, resized or deleted — that was the explicitly rejected alternative, because no square asset exists in the repo and drawing one is design work, not an audit fix. So: the application's own pixels do not change anywhere. What changes is what the browser is told about the install icon, and the fact that the banner is no longer offered up for maskable (safe-zone) cropping.
- [ ] **Confirm the source of truth agrees with itself.** `grep sizes src/manifest.webmanifest` must print exactly one line, containing `"sizes": "1916x484"`, and `grep -c maskable src/manifest.webmanifest` must print `0`. This pair is also pinned automatically by `src/app/api/pwa-manifest-icons.test.ts`, which reads every icon's `src` and compares the declared `sizes` string against the real IHDR width/height, so a future icon swap that forgets the manifest fails `npm run test`. Sanity-check that guard is real by temporarily editing the manifest to `"sizes": "512x512"`, running `npx vitest run src/app/api/pwa-manifest-icons.test.ts` (it must fail with `expected '512x512' to be '1916x484'`), then reverting the edit — do not commit that experiment.
- [ ] **The built artifact carries the corrected manifest.** `npm run build`, then `cat dist/gones/browser/manifest.webmanifest`. It must be byte-identical to `src/manifest.webmanifest` — `angular.json` copies it verbatim through an assets glob (`"glob": "manifest.webmanifest"`, `"input": "src"`, `"output": "."`), so there is no transform step that could rewrite the icons array. A stale `dist/` from before this commit will still show the old two-entry array; run the build first and do not judge this from an old tree.
- [ ] **DevTools → Application → Manifest no longer reports an icon size mismatch.** Serve the built app (`docker compose --profile release up --build -d`, SPA on `http://127.0.0.1:8081`, or `npm run dev` on `http://127.0.0.1:4200`), open the page, then DevTools → **Application** → **Manifest**. Scroll to the **Icons** section. Expected: exactly **one** icon rendered, described as `1916x484`, purpose `any`. Expected to be **gone**: the mismatch warning Chrome raised for each of the two old entries — wording varies by Chrome version, but it is of the form *"Actual size (1916×484) of icon http://127.0.0.1:8081/assets/gones_logo.png does not match specified size (192×192)"*, and the same again for `512×512`. If you still see a size warning, you are looking at a cached manifest — see the service-worker step below. Note the icon preview will look like a wide banner, not a square tile; that is the honest shape of the file and is the intended outcome of this change.
- [ ] **Check the install prompt, and report what you see rather than assuming it is fine.** In Chrome, look for the install affordance (the install icon at the right of the omnibox, or ⋮ → *Cast, save and share* → *Install page as app* / *Add to Home screen* depending on version). Install the app and look at the icon the browser generates for the launcher/taskbar/home screen. **This is the step with real residual risk in the chosen fix and the reason it is a manual check.** Declaring the true 3.96:1 banner means no square icon is declared at all, and install UIs handle that differently: some letterbox the banner into a square tile, some scale it, some fall back to a favicon-derived or letter-based icon. Any of those is an acceptable outcome here; what you must not accept silently is the app becoming **non-installable**. If the install affordance does not appear, DevTools → Application → Manifest has an **Installability** section that names the exact blocking reason (icon minimum-size rules are the plausible one, since Chrome requires a sufficiently large `purpose: any` icon) — copy that reason verbatim and report it, because it would mean the fix needs a real square asset after all rather than a metadata correction.
- [ ] **A previously installed copy will keep the old manifest until the service worker updates.** `ngsw-config.json` prefetches `/manifest.webmanifest` in its `app` asset group, so an already-installed PWA or a browser with the old service worker active can serve the stale two-entry manifest and show the old warnings even after you deploy. To see the change: DevTools → Application → Service Workers → **Unregister**, then Application → Storage → **Clear site data**, then hard-reload. For the installed-app case, uninstall the PWA first and install it again — an already-installed app's launcher icon is not re-read on every load.
- [ ] **Nothing in the visible UI moved.** The logo in the header (`src/app/app.component.ts`, the `.brand` link) and the logo on the About page (`src/app/features/menu/about.component.ts`) both point at the same `assets/gones_logo.png` and were not touched; they must render exactly as before, at the same size, with no layout shift. The browser tab favicon comes from `src/index.html:14` (`<link rel="icon" type="image/png" href="assets/gones_logo.png">`), which declares no `sizes` attribute and was deliberately left alone — the tab icon must look unchanged. If any of these three changed, something outside this slice's scope was modified.
- [ ] **The commit is the two files the ticket names plus this checklist.** `git show --stat HEAD` → `src/manifest.webmanifest`, `src/app/api/pwa-manifest-icons.test.ts` (new), `artifacts/manual_test_checklist.md`. Nothing else moved: `git show HEAD -- backend/ cypress/ ops/ deploy/ src/app/features/ src/app/app.component.ts src/index.html src/assets/ angular.json ngsw-config.json package.json` must print nothing — in particular `src/assets/gones_logo.png` must be untouched, since this fix deliberately creates no image. `git show HEAD -- src/manifest.webmanifest | grep '^-'` must show exactly the two removed icon lines. `npm run e2e:ci` and `npm run cy:run` were deliberately not run for this slice (plan Scope Out; their teardown destroys local database volumes); no Cypress spec asserts on manifest contents.
- [ ] **When you are done with the release container, stop it without destroying data.** Use `docker compose --profile release down` on its own. Do not add `--volumes` — that wipes the local database volumes and forces a full reseed.

## T34 daily-reconcile-ceiling

- [ ] **Read this first — what this commit changes and, more importantly, what happens to the work that no longer fits in one pass.** `TournamentScheduleReconciler.RefreshDailyAsync` used to do the entire daily reminder reconcile in **one** transaction, holding `pg_try_advisory_xact_lock(0x474F4E4553433237)` for the whole thing, with no ceiling on either the tournament set or the future-`ScheduledNotification` load. It now walks the same tournament set in **pages of at most `options.BatchSize`** (env `GONES_SCHEDULER_BATCH_SIZE`, default `100`, clamped `1..500`), each page in its own transaction under its own lock acquisition. **The ceiling bounds a page, not a run: `RefreshDailyAsync` loops until it gets a page shorter than `BatchSize`, so one call still covers every eligible tournament.** No row is skipped because a ceiling was hit. The rest of this section is mostly about proving that claim rather than taking it on trust, because a ceiling that silently drops rows would be worse than the unbounded loop it replaced.
- [ ] **Count how many tournaments the daily refresh is eligible to touch, so you can predict the page count.** With the local stack up (`docker compose up -d`), run:

  ```
  docker compose exec -T postgres psql -U gones_migration -d gones -At -c "
  SELECT count(*) FROM events e
  WHERE (e.status = 'Published' AND e.deleted_at IS NULL AND e.starts_at_utc > now())
     OR EXISTS (SELECT 1 FROM scheduled_notifications s WHERE s.event_id = e.id AND s.scheduled_at_utc > now());"
  ```

  Call that number `N`. That predicate is the page set verbatim from `RefreshDailyPageAsync`. The second arm (`EXISTS … scheduled_at_utc > now()`) is what keeps the daily *sweep* working for tournaments that were cancelled or soft-deleted but still carry future notification rows — without it, paging would have quietly stopped cancelling their reminders. For a batch size `B`, one `RefreshDailyAsync` performs exactly `floor(N / B) + 1` pages: full pages while at least `B` remain, then one final short (possibly empty) page that ends the loop. Seed some future events first (`npm run dev:env`) if `N` is 0 or 1 — you cannot observe paging with a set that never exceeds the ceiling.
- [ ] **Make the ceiling actually fire, and count the pages.** The compose `worker` service does not set `GONES_SCHEDULER_BATCH_SIZE`, so it runs at the default `100`; override it for a one-off run instead of editing `compose.yaml`:

  ```
  docker compose run --rm -e GONES_SCHEDULER_BATCH_SIZE=2 worker
  ```

  `Worker.cs` initialises `nextDailyPlan = Instant.MinValue`, so the daily refresh runs on the **first tick** after boot — you do not have to wait 24h. The worker runs with `GONES_OTEL_CONSOLE_EXPORTER: "true"`, so every `scheduler.plan` activity prints to its console. Let it complete one tick, then Ctrl-C and count: the number of `scheduler.plan` spans for that first tick must equal `floor(N / 2) + 1`. Before this commit it would have been exactly **1**, no matter how large `N` was — that single span is the finding this slice fixes. Pipe to a file if it scrolls: `docker compose run --rm -e GONES_SCHEDULER_BATCH_SIZE=2 worker > /tmp/t34-worker.log 2>&1` then `grep -c 'scheduler.plan' /tmp/t34-worker.log`.
- [ ] **Confirm every eligible tournament was planned by that single run — this is the "nothing was dropped" check and the most important step in this section.** After the run above, compare the eligible set against the set that actually has future planned rows:

  ```
  docker compose exec -T postgres psql -U gones_migration -d gones -c "
  SELECT count(*) AS eligible_without_planned_rows
  FROM events e
  WHERE e.status = 'Published' AND e.deleted_at IS NULL AND e.starts_at_utc > now()
    AND EXISTS (SELECT 1 FROM event_registration_attempts r WHERE r.event_id = e.id AND r.status = 'Confirmed')
    AND NOT EXISTS (SELECT 1 FROM scheduled_notifications s
                    WHERE s.event_id = e.id AND s.scheduled_at_utc > now() AND s.status = 'Planned');"
  ```

  It must print `0`. A non-zero count means a tournament with a confirmed, eligible registration was left unplanned after a run that returned "done", i.e. the ceiling dropped work — that is a hard failure of this commit, not a tuning issue. (Tournaments whose only registrations are cancelled, or whose registrant has an unconfirmed email or a closed profile, are correctly excluded by the query above, which mirrors the `PlanTournamentsAsync` candidate filter.)
- [ ] **Confirm the cross-page sweep still cancels — a tournament on page 2 must not keep stale reminders.** Pick a tournament that is *not* in the first page (order is `ORDER BY e.id`, so take one with a high uuid: `SELECT id FROM events ORDER BY id DESC LIMIT 1;` among the eligible set), cancel one of its confirmed registrations through the UI (or `UPDATE event_registration_attempts SET status = 'CancelledByUser' WHERE id = '…';`), then re-run the worker as above. Every `scheduled_notifications` row for that registration with `scheduled_at_utc > now()` must come back `Cancelled`. Before paging, one query swept all future rows at once; now the sweep is scoped to each page's tournaments, so this check is what proves the page set is wide enough to still reach them.
- [ ] **Prove that losing the advisory lock defers the remainder instead of dropping it.** In one terminal, take and hold the planner lock from a foreign session:

  ```
  docker compose exec -T postgres psql -U gones_migration -d gones -c \
    "BEGIN; SELECT pg_advisory_xact_lock(5138411759509647927); SELECT pg_sleep(45); COMMIT;"
  ```

  (`5138411759509647927` is `0x474F4E4553433237`, the `PlannerAdvisoryLock` constant.) While that is held, start the worker. Its first page cannot take the lock, so `RefreshDailyAsync` returns `false` and **no** `scheduler.plan` span appears for that tick. This is the intended, pre-existing replica-safety contract: `Worker.cs:47` only advances `nextDailyPlan` when the call returns `true`, so a `false` leaves the daily refresh due and the next tick retries the whole thing from the start. Let the psql session finish (or Ctrl-C it) and watch the next tick: the spans appear and the "eligible_without_planned_rows" query above returns to `0`. Deferred, then completed — never skipped.
- [ ] **Prove the same for a lock lost *between* pages, which is the case that is genuinely new in this commit.** Set the ceiling to `1` so the loop is long and easy to interrupt (`docker compose run --rm -e GONES_SCHEDULER_BATCH_SIZE=1 worker`), and while the tick is running, grab the lock from a second terminal with the `pg_advisory_xact_lock` command above. Expect: the worker emits **some but not all** of the `floor(N / 1) + 1` spans for that tick, the "eligible_without_planned_rows" query temporarily reports a **non-zero** count, and — after you release the lock — the very next tick brings it back to `0`. That transient non-zero is the honest behavior of this design and is exactly what you must confirm recovers: pages already committed stay committed, the remaining tournaments are simply not visited on that pass, and because the run returned `false` the worker re-runs the whole refresh rather than resuming from a saved cursor. The same reasoning covers a worker crash or `docker compose restart worker` mid-loop — a partially applied refresh is a valid state that the next run reconciles. If that count does *not* return to `0` on a subsequent tick, stop and report it.
- [ ] **Confirm the pending-changes path is untouched.** `RefreshPendingChangesAsync` was already capped (`.Take(options.BatchSize)` on `event_lifecycle_entries`) and this commit only restructured it around the shared `PlanTournamentsAsync` helper — it still runs as a single transaction under a single lock acquisition, with no paging. Exercise it by making a major date change to a published tournament with confirmed registrations (change its date through the organizer UI), then watch a worker tick that is *not* the daily one: the old future rows for that tournament flip to `Cancelled`, new `Planned` rows appear on the new date, and the `event_lifecycle_entries` marker gets a non-null `reminder_plan_processed_at`. Exactly one `scheduler.plan` span for that tick.
- [ ] **Note the observability semantics change before you judge a dashboard.** `metrics.RecordPlan` still feeds the `gones.scheduler.planned` and `gones.scheduler.cancelled` **counters**, and counters sum across pages, so totals over any time window are unchanged. What did change is the `scheduler.plan` **activity**: there is now one span per page instead of one per daily refresh, and its `scheduler.planned` / `scheduler.cancelled` tags are that page's counts, not the whole refresh's. If a saved trace view or alert reads a single span's tag as "how much the daily refresh did", it will now under-report; read the counters instead.
...
## T37 cypress-deterministic-wait

Spec touched by this slice: **`cypress/e2e/power-user-gating.cy.js`** — the only one. Watch it specifically on the end-of-plan e2e run.

**Nothing below was executed by the agent.** Running Cypress is plan Scope Out for this slice (`npm run e2e:ci` teardown runs `down --volumes` and destroys the local DB volumes), so the change was verified by static means only: `node --check`, greps, a read of the diff against the frozen contract, and the four repo gates. The end-of-plan `npm run e2e:ci` is what actually proves it.

- [ ] **Run the end-of-plan e2e gate and read the `power-user-gating.cy.js` line in the summary.** `npm run e2e:ci` prints a `=== e2e specs: N/N passed ===` block at the end (`scripts/full-stack-ci.mjs`); `PASS  cypress/e2e/power-user-gating.cy.js` is the line this slice is responsible for. All five of its tests must pass, but the one that changed is `keeps an anonymous local Live detail readable while mutations are off`.
- [ ] **Confirm the changed test no longer sleeps.** In that test, the fixed `cy.wait(600)` between the name edit and the `cy.reload()` was replaced by `expectPersistedLiveName('Local Power Cup')`, a bounded poll that reads IndexedDB database `gones-live`, object store `tournaments`, and proceeds the moment a committed row carries the new name. In the Cypress command log for that test you should see a short run of `window` + `wait 150ms` pairs (usually one or two, sometimes zero) instead of a single flat 600 ms `wait` — that is the gate doing its job. `grep -n "cy.wait(600)" cypress/e2e/power-user-gating.cy.js` must print nothing.
- [ ] **Know the exact failure signature if persistence stalls.** The poll is capped at 40 attempts × 150 ms = a 6 s budget. On exhaustion the test fails with `IndexedDB 'gones-live'/'tournaments' row named "Local Power Cup" (poll 40): expected 40 to be below 40`. If you see that message, the finding is a real persistence regression in the local Live write path (`blur` → `debounceIntent('settings', 400 ms)` → `pumpIntents` → `LocalLiveBackend.updateLiveSettings` → IndexedDB `put`) — **not** a flaky wait. Before this commit the same regression would have shown up as the far less informative `cy.contains('h1', 'Local Power Cup')` failing after the reload, or would have passed silently whenever it happened to land inside 600 ms.
- [ ] **Confirm the two sibling repairs from earlier tickets in this batch still hold.** This slice deliberately did not touch them, and both are themselves unverified by execution until this run: `cypress/e2e/event-registration.cy.js` (paginated participants fixture, `participants*` glob, a `public-participants-show` click), and the `'**/api/events/lyon-legacy/participants*'` intercept returning `{ items: [], page: 1, pageSize: 20, totalCount: 0 }` in both `cypress/e2e/accessibility.cy.js` and `cypress/e2e/abuse-surface.cy.js`. If any of those three go red, the cause is the earlier ticket, not T37.
- [ ] **Confirm no app behavior changed.** The commit touches one Cypress spec plus this checklist. `git show --stat HEAD` must list exactly `cypress/e2e/power-user-gating.cy.js` and `artifacts/manual_test_checklist.md`, and `git show HEAD -- src/ backend/ ops/ deploy/ scripts/ compose.yaml package.json cypress.config.js` must print nothing. In particular the 400 ms debounce and the IndexedDB schema are untouched: the spec now *observes* that chain instead of guessing how long it takes.
- [ ] **No stack teardown beyond stopping what you started.** If you stop the stack after the e2e run, use `docker compose down` on its own: do not add `--volumes` and do not run `npm run db:reset`. Either wipes the local database volumes.

## T38 remove-dead-exports

This slice deletes only code that nothing calls: seven unreferenced TypeScript exports (plus two private helpers reachable only from one of them) and two whole files. **No runtime behavior should change anywhere.** The point of the checks below is therefore inverted from a normal section — you are confirming that the surfaces *nearest* the removals are untouched, not that a new behavior appeared. Anything that looks different is a regression.

The frozen symbol list was re-derived against `HEAD` before any deletion (37 commits had landed since the ticket was written): all nine candidates were still consumer-free, none was dropped, none was added. Frontend gates, backend gates, and a production bundle build all ran green; the two `NG8113 RouterLink` warnings in `AdminOrganizationsComponent` / `AdminUsersComponent` are pre-existing and belong to untouched files.

- [ ] **Confirm the Archive tournament detail page still validates and warns.** `src/app/domain/validation.ts` and `src/app/domain/warnings.ts` each lost one export, but their live consumer `tournament-detail.component.ts` imports `validateRoundEntry` (line 36) and `getTournamentWarnings` / `TournamentWarning` (line 37) — both kept. Open any archived tournament, edit a round entry into an invalid state (blank player, a player facing themselves, a game score above 2, the reserved name `bye`) and confirm the same inline validation messages appear as before. Then open a tournament missing a bye in an odd-sized round and confirm the warning banner still lists it. The deleted `isValidRoundEntry` and `hasMissingByeWarning` were thin one-line wrappers over exactly these two functions and had no caller.
- [ ] **Confirm player rename and merge still work in Settings.** `src/app/domain/rename-player.ts` lost its Live-tournament rename slice, but the Archive path is untouched: `renamePlayerInTournament` (used by `local-archive-backend.service.ts:380`), `renamePlayerInRoundEntry`, `playerNameKey`, and `samePlayerName` all survive byte-identical. In Settings, rename a player who appears in several tournaments and confirm every round entry and the deck-archetype rows follow. Then rename player A onto an existing player B to trigger the **merge** dialog and confirm the merged rows behave as before (existing non-empty archetype wins).
- [ ] **Confirm Live tournaments are unaffected — and understand why there is nothing to click.** The deleted `renamePlayerInLiveTournament` (with its `renameLivePlayer` / `renameLiveRound` helpers) had **no caller anywhere**: there is no rename-player control on the Live runner, so this removed an unwired capability, not a feature. Open a Live tournament, add players, enter results, and reload: everything must behave exactly as before. If you ever *do* find a rename control on a Live tournament, stop — that would mean the grep missed a consumer and this deletion must be reverted.
- [ ] **Confirm login and registration still show field-level errors.** `src/app/auth/auth-errors.ts` lost `genericAuthError` but kept `fieldErrorsFromProblem` and `AuthFieldErrors`, which are what `auth-entry.component.ts` and `account-settings.component.ts` actually import. Submit the login form with a bad password, then the register form with an already-used email and a too-weak password, and confirm the per-field messages render as before. Also trip the rate limiter (repeated bad logins) and confirm the throttling message you get today is unchanged — the deleted function contained its own unused copies of the 401 and 429 strings, which is precisely why it was dead. The translation key `auth.genericError` is a **different** thing and is still in use; do not confuse the two.
- [ ] **Confirm the admin list pages still paginate and cache.** `src/app/features/admin/admin-query.ts` lost only `ADMIN_CACHE_FAMILIES`, an unused constant. `totalPages`, `adminCacheKey`, `pagedQueryParams`, and `readPagedQuery` are all kept and are what the six admin components import. Page through Admin → Users, Organizations, Organization members, Audit, Notification delivery, and Deleted events: page counts, page-size changes, and the URL query round-trip must all behave as before, and revisiting a page you already loaded should still hit the cache rather than refetch.
- [ ] **Confirm the public Events catalog still caches for 24 hours.** `event-catalog-cache.service.ts` lost the `EVENT_CATALOG_TTL_MS` re-export, but freshness never went through it: `isCatalogFresh` defaults to `CATALOG_TTL_MS` from `src/app/shared/catalog-cache.ts`, which is unchanged. Load `/events` while signed out, reload and confirm it serves from cache without a network round-trip, then confirm a forced refresh revalidates via `If-None-Match` and a `304` keeps the cached items. **The import of `../../shared/catalog-cache` was deliberately kept** (only the one unused specifier was dropped) because `server-authority-boundary.test.ts` asserts the exact set of files importing that helper — if that gate ever goes red here, the import clause was removed, not just the specifier.
- [ ] **Confirm the League Archive list cards still print correct tournament and player counts.** The deleted `backend/src/Gones.Domain/Leagues/LeagueCatalogCounts.cs` was superseded by `backend/src/Gones.Domain/Archive/ArchiveCatalogCounts.cs`, which is what actually stamps the counters today (`ArchiveLeagueSeason.cs:59,119,124`, `ArchiveTournament.cs:55,102,136`, `ArchiveTournamentCommandEndpoints.cs:405`, `LiveCommandEndpoints.cs:401`, `MigrationImportService.cs:151`). Browse the League Archive list and spot-check two Seasons: the tournament count and player count on each card must match what you see after opening the Season. Then archive a Live tournament into a Season and confirm the card's counters refresh — that is the `RefreshCatalogCounts` path, which never touched the deleted class.
- [ ] **Confirm exports and downloads still produce files.** `src/app/shared/save-text-file.ts` was deleted whole; nothing imported it. The download paths still in use are `src/app/shared/save-json-file.ts` and the inline anchor code in `tournament-result.component.ts:292-295` and `organizer-participants.component.ts:331-334`. Exercise each: the full JSON export from the app menu, the tournament result image/SVG export, and the organizer participants CSV download. Each must still save a correctly named, non-empty file.
- [ ] **Note the two stale documentation references left on purpose.** `docs/adr/0042-slim-league-archive-catalog.md` (lines 45, 86) and `docs/league-archive-catalog-summary.html` (lines 105, 128) still name `LeagueCatalogCounts`. These are historical decision records, not consumers, and editing them was out of this ticket's removal set. If you want them re-anchored to `ArchiveCatalogCounts`, that is a separate docs change.
- [ ] **The commit is the eight files the ticket names plus this checklist.** `git show --stat HEAD` → deleted `src/app/shared/save-text-file.ts` and `backend/src/Gones.Domain/Leagues/LeagueCatalogCounts.cs`; modified `src/app/domain/rename-player.ts`, `src/app/auth/auth-errors.ts`, `src/app/domain/validation.ts`, `src/app/domain/warnings.ts`, `src/app/features/admin/admin-query.ts`, `src/app/features/events/event-catalog-cache.service.ts`, plus `artifacts/manual_test_checklist.md`. The diff must be deletion-only apart from exactly **one** inserted line — the rewritten import in `event-catalog-cache.service.ts`. `git show HEAD -- cypress/ ops/ deploy/ scripts/ fixtures/ compose.yaml package.json` must print nothing; in particular `fixtures/archive-domain/v5/parity/manifest.json` is untouched, because the parity hash covers the generated fixture document and none of the deleted symbols is invoked while producing it.
- [ ] **No stack teardown beyond stopping what you started.** `npm run e2e:ci` and `npm run cy:run` were deliberately not run for this slice (plan Scope Out). If you stop the stack, use `docker compose down` on its own: do not add `--volumes` and do not run `npm run db:reset`. Either wipes the local database volumes.

## T39 orphan-i18n-keys

This slice deletes **112 message keys from each catalog** (224 lines, EN + FR) in `src/app/i18n/messages.ts` and adds a fourth test to `src/app/i18n/message-namespace.test.ts` that fails whenever a catalog key is never referenced outside the catalog. **No rendered string should change anywhere** — every deleted key was unreachable. As with T38 the checks below are inverted: you are confirming that nothing moved, and in particular that **no surface falls back to showing a raw key or an empty string**.

The deletion list was **re-derived at `HEAD`, not taken from the plan snapshot** (45 commits had landed since the ticket's `ee3c41b` anchor). The same scan run against `ee3c41b` and against the audit snapshot `50e4a00` returns the identical 112 keys, so nothing that was live at snapshot time was deleted; the four keys added by earlier tickets in this batch were each confirmed still referenced from real component code before anything was removed. `npm run typecheck`, `npm run lint`, `npm run test` (164 files / 2089 tests), `npm run backend:test` (336 unit + 20 architecture + 649 integration) and a production `npm run build` all ran green. The two `NG8113 RouterLink` warnings in `AdminOrganizationsComponent` / `AdminUsersComponent` are pre-existing and belong to untouched files.

- [ ] **Switch the UI to French and walk the main surfaces looking for raw keys and blank text.** Go to `/settings`, open the language selector (`settings-language-select`) and choose *Français*. Then visit, in order: `/` (home menu), `/about`, `/events`, one event detail `/events/:slug`, `/registrations`, `/archive/league-seasons`, `/archive/tournaments`, one Season `/archive/league-seasons/:seasonId`, one tournament `/archive/tournaments/:tournamentId`, its `/result` and `/result/metagames` tabs, `/live-tournaments`, one Live tournament runner, `/global-stats`, `/settings`, `/settings/account`, and `/login`. **On every page: no dotted identifier may be rendered as visible text** (the translate fallback returns the key itself when a lookup misses, so a deleted-but-still-used key would surface as literal text like `event.eventName`), and **no label, button, heading, tooltip or aria-label may be empty**. Check headings, buttons, table column headers, empty states and error banners. Then switch back to English via the same selector and confirm the same pages are unchanged.
- [ ] **Confirm the retired rich-text event editor is genuinely gone, not merely unstyled.** The largest deleted cluster is the old event editor's toolbar and image prompts — `event.bold`, `event.italic`, `event.paragraph`, `event.richTitle`, `event.richToolbarAria`, `event.editorAria`, `event.plainSummary`, `event.imagePrompt`, `event.imageAltPrompt`, `event.imageUrl`, `event.badImageUrl`, `event.externalLink`. Open `/events/new` and `/organizer/events/:id/edit` as an organizer and confirm the summary field is the plain textarea it is today, with its own character-limit message, and that there is no formatting toolbar. If a toolbar appears with blank buttons, revert: the deletion missed a live consumer.
- [ ] **Confirm event dates still render localised weekday names.** All seven `event.weekday.mon` … `event.weekday.sun` keys were deleted because weekday rendering moved to locale-aware formatting. On `/events` and on an event detail page, in **both** languages, confirm the day names read as real words (Monday / lundi, Saturday / samedi), not as three-letter stubs and not as raw keys. Compare an event that spans a weekend against the same event in the other language — the two must differ.
- [ ] **Confirm the calendar download and "edit in calendar" affordances are absent as expected.** `event.downloadIcs`, `event.downloadAllFailed`, `event.downloadOneFailed`, `event.downloadEventLabel`, `event.editInCalendar`, `event.archiveTournament`, `event.liveTournament`, `event.viewPage`, `event.eventPage` and `event.addEventLabel` were all deleted. Browse `/events` and an event detail page in both languages and confirm there is no ICS/calendar download button and no "edit in calendar" link. Their absence is the pre-existing state, not something this commit caused — but a **blank** button where one of those used to be would be a regression.
- [ ] **Confirm the Settings migration-bundle exporter is still absent.** Fourteen `settings.migration*` keys (title, help, warning, counts, hash, instance, versions, download, failed and their label variants) belonged to the retired migration-bundle exporter. Open `/settings` in both languages and confirm there is no migration panel, and that the export/import controls that *do* exist still work: run the full JSON export and re-import it, and confirm the success and failure messages are complete French sentences with no dotted identifiers.
- [ ] **Confirm the Archive tournament round/match delete dialogs still read correctly.** `tournament.deleteRoundTitle`, `tournament.deleteRoundMessage`, `tournament.deleteRoundConfirm` and the three matching `tournament.deleteMatch*` keys were deleted, but the **live** menu-item label **tournament.deleteRound** survives and is what `tournament-detail.component.ts:256` renders. This is the trap case: a bare substring scan would have deleted the surviving key too. Open an archived tournament in French, use the round menu → delete a round, and confirm both the menu item and the confirmation dialog show real text with a real title, body and confirm button. Do the same for deleting a match.
- [ ] **Confirm the Live runner's starting-record editor still labels its fields.** `live.startingWinsFor`, `live.startingLossesFor`, `live.startingDrawsFor` and `live.startingRecordFor` were deleted. Open a Live tournament, add a player and set a starting record. In French, every input in that control must carry a visible label or an accessible name — inspect with the browser's accessibility panel and confirm no field's accessible name is empty or a dotted identifier.
- [ ] **Confirm the sortable tables still announce sort direction.** `globalStats.sortAsc` and `globalStats.sortDesc` were deleted. On `/global-stats`, and on the two Archive lists `/archive/league-seasons` and `/archive/tournaments`, toggle each sortable column header in French and confirm the direction is still conveyed — visually by the arrow and to a screen reader by the header's `aria-sort` / the direction-toggle's aria-label. The Archive lists compose their sort labels through the archiveTournaments.sort\* and archive.sort\* maps, which are untouched.
- [ ] **Confirm the Account settings page labels every field.** Eight `profile.*` keys were deleted (`profile.title`, `profile.back`, `profile.location`, `profile.linkHelp`, `profile.currentPasswordOptional`, `profile.currentPasswordUsername`). Three of them are near-misses of keys that are still live and still rendered — profile.locationCountry, profile.locationRegion and profile.locationCity on `/settings/account`, and account.linkHelp for the account-linking help text. Open `/settings/account` in French and confirm the country, region and city fields all have visible labels, the account-linking help paragraph is present, and the change-password section still explains when the current password is required.
- [ ] **Confirm the app header and the participants, player and result surfaces are intact.** Singles deleted here: `header.settings` (whose near-miss header.settingsActionsAria is live and supplies the header actions aria-label on every page), `registration.title`, `participants.selected`, `player.byes`, `result.archetype`, `leagues.back`, `leagues.crossAuthorityMove`, and seven `common.*` words (`common.vs`, `common.versus`, `common.round`, `common.match`, `common.hours`, `common.description`, `common.allDay`). In French, check: the header action cluster still exposes an aria-label (accessibility panel, any page); `/organizer/events/:id/participants` still labels its selection controls; a player detail page still shows the byes column with a header; and a tournament result page still shows the archetype column with a header.
- [ ] **Understand what the new guard does and does not cover, before you add a key.** The new test `every key is referenced outside the catalog` scans every file listed by `git ls-files` except the catalog and requires each EN key to appear wrapped in a quote, a double quote or a backtick. **Consequence: naming a live key inside backticks in a Markdown file — including this checklist — counts as a reference and will hide a real orphan.** That is why this section writes surviving key names bare and only backticks keys that were deleted. Exactly one key is currently held alive by documentation alone rather than by code: event.tournamentEvent, referenced only from `docs/adr/0035-calendar-event-vocabulary.md`. It was **not** deleted, because the ticket's rule is to delete exactly what the scan reports and no more. If that key is meant to go, it needs its own change that also updates the ADR.
- [ ] **Know the allowlist and when to extend it.** Keys composed at runtime can never be proved used by a literal scan, so `message-namespace.test.ts` carries a commented `DYNAMIC_KEY_PREFIXES` allowlist holding exactly two regexes, for the eventManage.major.\* and eventManage.field.\* families built in `organizer-event-create.component.ts:498` and `:578`. Nothing else in the repo composes a message key. If you ever add a third composing call site, add its prefix there **and** say so in the commit body — otherwise the new family's keys will be reported as orphans and someone will delete live strings.
- [ ] **The commit is two source files plus this checklist.** `git show --stat HEAD` must list exactly `src/app/i18n/messages.ts`, `src/app/i18n/message-namespace.test.ts` and `artifacts/manual_test_checklist.md`. The `messages.ts` half of the diff must be **deletion-only** — 224 removed lines, zero insertions, zero modified translation strings — and `git show HEAD -- src/app/features src/app/auth src/app/shared src/app/domain backend/ cypress/ ops/ deploy/ scripts/ package.json` must print nothing. If any surviving translation string changed, that is out of this ticket's scope and must be reverted.
- [ ] **No stack teardown beyond stopping what you started.** `npm run e2e:ci` and `npm run cy:run` were deliberately not run for this slice (plan Scope Out); the end-of-plan e2e gate covers it. If you stop the stack, use `docker compose down` on its own: do not add `--volumes` and do not run `npm run db:reset`. Either wipes the local database volumes.

## T40 e2e-frontend-flag-exports

This slice adds **four keys and nothing else** to the `composeEnv` literal in `scripts/full-stack-ci.mjs` — `GONES_FRONTEND_AUTH_V1`, `GONES_FRONTEND_ADMIN_V1`, `GONES_AUTH_V1` and `GONES_ADMIN_V1`, all `'true'`. It repairs a regression this same batch introduced: T15 (`0d45363`) correctly stopped `frontend-release` from reading the backend-shaped `GONES_FEATURES__*` names and switched it to the four documented frontend names with `false` defaults (`compose.yaml:159-160` build args, `compose.yaml:167-168` container environment) — but the e2e script never exported those names, so post-T15 all four fell back to `false` and the released bundle served auth and admin **off**.

**Why this ticket exists: it unblocks the plan's single end-of-run `npm run e2e:ci` human step.** With the flags off, `src/app/app.routes.ts:78,95,96` spreads the auth, registration/organizer and admin route sets in only when the auth and admin capability flags are true, so every gated URL falls through to the `**` catch-all at `src/app/app.routes.ts:98` and renders `NotFoundComponent`. Without this fix the end-of-run e2e run would be **expected-red for reasons unrelated to any audit finding**, which destroys its value as the batch's final check. No runtime application code was touched by this slice; the only change is the environment a dev-only script hands to `docker compose`.

Verified before commit, without bringing any container up: the rendered release config (`docker compose --profile release config frontend-release`) showed all four names as `"false"` before the edit and all four as `"true"` after, with the after-render driven by the four values parsed straight back out of the edited script — byte-identical to the hand-typed render. `node --check scripts/full-stack-ci.mjs` exits 0, `npm run typecheck` and `npm run lint` are clean, `npm run test` is 164 files / 2089 tests green, and `npm run backend:test` is 336 unit + 20 architecture + 649 integration green.

- [ ] **Read this before you run the e2e gate — `npm run e2e:ci` destroys your local database volume.** Its own `finally` block runs `docker compose --profile release down --volumes --remove-orphans` (`scripts/full-stack-ci.mjs:130`), and `--volumes` removes the project's `postgres-data` volume (`compose.yaml:19,181`), which is the same volume your day-to-day dev Postgres uses. This is pre-existing behaviour that this slice did not introduce or change, but it is the command the plan asks you to run, so decide deliberately: back the volume up first, or accept that you will re-seed afterwards. Do not run it casually on a database whose contents you care about.
- [ ] **Confirm the fix without starting anything (30 seconds, zero side effects).** Run the render on its own — `env GONES_FRONTEND_DATA_MODE=server GONES_FRONTEND_AUTH_V1=true GONES_FRONTEND_ADMIN_V1=true GONES_AUTH_V1=true GONES_ADMIN_V1=true GONES_FEATURES__AUTH_V1=true GONES_FEATURES__ADMIN_V1=true GONES_FEATURES__CALENDAR_V1=true GONES_FEATURES__LEAGUE_SERVER=true GONES_FEATURES__LIVE_SERVER=true GONES_AUTH_PROVIDER=Local GONES_AUTH_RATE_LIMIT_PERMIT_LIMIT=1000 GONES_FRONTEND_API_BASE_URL=http://127.0.0.1:5080 docker compose --profile release config frontend-release`. The `frontend-release` block must show `GONES_FRONTEND_AUTH_V1: "true"` and `GONES_FRONTEND_ADMIN_V1: "true"` under `build.args`, and `GONES_AUTH_V1: "true"` and `GONES_ADMIN_V1: "true"` under `environment`. `config` renders only — it starts no container and touches no volume.
- [ ] **Run the batch's end-of-run e2e gate and confirm the ten flag-gated specs reach their components.** `npm run e2e:ci`. These ten specs visit at least one route that the flags gate, and every one of them lands on the not-found page when the flags are off — they are the direct evidence that this fix worked: `cypress/e2e/auth-profile.cy.js`, `cypress/e2e/auth-route-guards.cy.js`, `cypress/e2e/auth-session-persistence.cy.js`, `cypress/e2e/admin-orgs.cy.js`, `cypress/e2e/admin-notification-delivery.cy.js`, `cypress/e2e/organizer-event-create.cy.js`, `cypress/e2e/organizer-event-management.cy.js`, `cypress/e2e/organizer-participants.cy.js`, `cypress/e2e/event-registration.cy.js` and `cypress/e2e/abuse-surface.cy.js`. The run prints a `=== e2e specs: N/M passed ===` summary at the very end — read that summary rather than scrolling the log. A **not-found page** in a failure screenshot or a `cy.visit` that lands nowhere means the flags did not reach the bundle; anything else is an ordinary spec failure to triage on its own merits.
- [ ] **Prove the flags actually reached the browser, not just the compose file.** While the release stack is up, `curl -s http://127.0.0.1:8081/runtime-config.json`. It must report `dataMode` of `server`, the API base URL, and both capability flags as `true`. That document is rendered at container start by `deploy/nginx/gones-runtime-entrypoint.sh:37` from the container environment, served from the tmpfs via `deploy/nginx/default.conf.template:20-21`, and read by `src/main.ts:44` before Angular bootstraps — so it is the authoritative declaration. If it reports either flag as `false`, the container environment is wrong and no amount of rebuilding the bundle will help.
- [ ] **Walk the three gated route families by hand and confirm none of them 404s.** With the stack up, open `http://127.0.0.1:8081` and visit `/login`, `/register` and `/settings/account` (auth), `/registrations`, `/events/new` and `/organizer/events` (registration + organizer), then `/admin`, `/admin/users` and `/admin/audit` (admin). Signed out you should get the real page or a guard redirect to the sign-in flow — what you must **not** get is the not-found component, which is precisely the pre-fix symptom. Compare against a control: `/definitely-not-a-route` must still render not-found, otherwise you are looking at the wrong page.
- [ ] **Confirm the API side is untouched.** The five `GONES_FEATURES__*` entries in the script are deliberately unchanged — the API container still consumes them, and they are what turn on the backend halves of auth, admin, calendar, league and live. With the stack up, sign in through the UI and load one admin list and one organizer page end-to-end. If the frontend now renders the admin route but every API call behind it 403s or 404s, the backend flags regressed, which would be a different bug than the one this slice fixed.
- [ ] **Always pass `--build` if you bring the release stack up by hand.** The two `GONES_FRONTEND_*` names are **build arguments**: `Dockerfile:34-40` bakes them into `src/environments/environment.prod.ts` with `sed` at image build time. The script always uses `up --build`, so changing the argument invalidates that layer and the bundle is rebuilt. A hand-run `docker compose --profile release up -d` without `--build` can serve a stale image built when the flags were still `false`, and you will then be debugging a cached artifact rather than the fix. The baked value is only the fallback the bundle ships with — the injected runtime declaration wins when it is present (`src/main.ts:41-54`) — but a stale image is still a real way to see the old behaviour.
- [ ] **Confirm no other compose invocation changed, and understand why yours might still show `false`.** `git show HEAD -- compose.yaml compose.release-test.yaml compose.release-candidate.yaml cypress/ src/ backend/ package.json` must print nothing. The base file's `false` defaults are T15's deliberate fail-closed choice and stay: a bare `docker compose --profile release up` with no environment still renders `false`, and that is correct, not a regression. If you want the flags on outside the e2e script, copy the four lines from the example environment file at the repository root (lines 6-7 and 15-16) into your own local one — they are already documented there as `true`. The two release overlays already default to `true` on their own (`compose.release-test.yaml:195-203`, `compose.release-candidate.yaml:35-36`).
- [ ] **Do not enable admin without auth.** `src/app/config/data-authority.ts:66` throws `serverModeAdminRequiresAuth` when the admin flag is set without the auth flag, and `deploy/nginx/gones-runtime-entrypoint.sh:22` refuses to start the container on an unsatisfiable pair. This slice sets both to `true` together, so the pair is valid — but if you experiment with the four names by hand and see the frontend container exit at startup or the app render a configuration-failure screen instead of the UI, read the container log: it names the exact failing combination.
- [ ] **The commit is one script plus this checklist.** `git show --stat HEAD` must list exactly `scripts/full-stack-ci.mjs` and `artifacts/manual_test_checklist.md`. The script half of the diff must be **four inserted lines and zero deletions**, all inside the `composeEnv` literal, immediately after the data-mode entry. No compose file, no cypress spec, no other script, and no application source may appear. No new capability flag was introduced: all four names already existed, were already documented in the example environment file and in `DEPLOYMENT.md:25-26,34-35,110-112`, and were already read by the compose file — the script simply stopped omitting them.

## T41 generation-ownership-reanchor

This slice closes the F1 residual T30 measured but could not fix inside its fence: two tabs restoring the **same** account concurrently used to leave one of them reporting a signed-out session. The origin-wide generation counter advances on every establishment, and the losing tab asserted an exact generation match from *outside* the lock (it must — `GET /api/users/me` is a network round trip), so it superseded itself. The fix records **who** owns a generation next to the counter: `advanceGeneration()` now also writes `localStorage['gones.auth.sessionOwner']` as `<generation>:<accountId>` in the very same lock hold that moves the counter and publishes the token, where the account id is the `sub` claim of the access token just minted. A tab whose generation moved re-anchors onto the new one **only** when that record is an exact string match for its own profile id; an unknown, missing, unreadable or foreign owner supersedes exactly as before. Gates at commit time: typecheck 0, lint 0, vitest 164 files / 2091 tests, backend 336 unit + 20 architecture + 649 integration.

- [ ] **Know what this slice put in browser storage before you test it.** From this commit on, one account id — your user GUID, the same one `GET /api/users/me` returns — is stored in `localStorage` under `gones.auth.sessionOwner`, in the form `<generation>:<accountId>`, for as long as that account's session lives on this browser. It is removed on logout and on any session teardown (`invalidateSession()`), so it does not outlive a clean sign-out; a browser closed without signing out keeps it until the next establishment or teardown, which is the same window in which the refresh cookie still exists. The access token itself is still never persisted. Nothing else may ever be written to that key — no email, no username, no scope data. This is a deliberate, reviewed relaxation of the "markers and counters only" rule for `gones.auth.*`, and the architecture guard in `src/app/backend/server-authority-boundary.test.ts` is what holds it to exactly that one identifier.
- [ ] **Same account, two tabs, cold start — both must stay signed in.** This is the regression the slice exists to fix. Sign in normally, then quit the browser entirely (leave the session alive — do **not** sign out) and reopen it with two tabs on the app restoring at the same time; the reliable way is to set the browser to reopen the previous session with two app tabs, or to open the app in two tabs and hit reload on both within the same second. Both tabs must land on the signed-in UI with your username in the header. Before this fix, exactly one of them would show the signed-out state and need a manual reload. If either tab shows signed-out, capture `localStorage['gones.auth.sessionGeneration']` and `localStorage['gones.auth.sessionOwner']` from both tabs' consoles before reloading — that pair is the whole diagnosis.
- [ ] **Same account, two tabs — confirm the shared session is real, not cosmetic.** With both tabs signed in from the step above, run a private read in each (open `/settings/account`, then a registrations or organizer page you have access to). Both must load your data without a 401 loop and without bouncing to the sign-in page. Then check the console in both tabs: `localStorage['gones.auth.sessionOwner']` must read `<n>:<your account id>` with the same `<n>` as `localStorage['gones.auth.sessionGeneration']`. Two different generations, or an owner naming an id that is not yours, means the re-anchor bound the wrong scope — treat that as a stop-ship and report it.
- [ ] **Different accounts — the superseded tab must NOT show the other account's data.** This is the safety property; run it even if you skip everything else. Open tab A and tab B on the app. In tab A, trigger a restore (reload it) and, while it is still loading, sign in as a **second, different account** in tab B. Tab B must end up signed in as the second account. Tab A must end up **signed out** — a sign-in prompt or an anonymous page. What it must never show is the second account's username, email, registrations or admin surfaces, and it must never show your first account's private data either. Then reload tab A: it must come back as the second account (the one that owns the browser session now), never as a mix of the two.
- [ ] **Different accounts — confirm nothing of the losing account was cached.** Right after the previous step and before any reload, in tab A's console read `localStorage['gones.auth.sessionOwner']`: it must name the **second** account's id, not the first. Then open DevTools → Application → IndexedDB and confirm the per-user store holds rows for the second account only. Any row keyed to the first account's id that appeared during that window is a cross-account scope leak — stop and report it.
- [ ] **Single tab is unchanged.** Sign in, reload the page with the session live, then sign out. Sign-in lands on the app signed in, the reload restores the session without a flash of signed-out UI, and sign-out clears it. After sign-out, `localStorage` must contain **no** `gones.auth.sessionOwner` key at all — teardown removes it, which is why an account id may live there only as long as that account's session does.
- [ ] **Storage denied — sign-in must fail closed, not half-work.** In a private window with site data blocked for the origin (or with `localStorage.setItem` throwing via a DevTools override), attempt to sign in. It must fail with the coordination-unavailable path — no signed-in UI, no access token retained — exactly as before this slice. The owner record is written with the same fail-loud semantics as the generation counter, so a store that cannot be written must not produce a half-established session.
- [ ] **Old browser profile, first read of the new key.** On a browser that already has a session from a build before this slice, `gones.auth.sessionOwner` does not exist yet. Load the app: the restore must work normally. The missing record is read as "unknown owner", which fails closed — it can only cost the re-anchor on a concurrent restore, and it must never throw or block the bootstrap. Nothing needs migrating; the first establishment writes the record.
- [ ] **The commit is four files.** `git show --stat HEAD` must list exactly `src/app/auth/auth-session-coordination.service.ts`, `src/app/auth/auth.service.ts`, `src/app/auth/auth.service.bootstrap.test.ts`, `src/app/backend/server-authority-boundary.test.ts` and `artifacts/manual_test_checklist.md`. The fourth is the architecture guard that pins which `localStorage` operations the coordination service is allowed to make: it enumerates them with exact counts, so a new key cannot be added without a deliberate edit there. Its allowlist gained the three `AUTH_SESSION_OWNER_KEY` operations and its storage-surface comment now names the account id — that edit is the record that the "markers and counters only" line was relaxed on purpose, for one identifier, removed on teardown.
- [ ] **Known gap, no action needed unless you change the fixture.** `cypress/e2e/offline-public-read.cy.js` asserts the auth `localStorage` surface is exactly `['gones.auth.sessionGeneration']`, and it still passes because its intercepted refresh returns the literal string `memory-token` rather than a JWT, so no `sub` claim is readable and no owner record is written. If you ever make that intercept return a realistic signed token, that assertion will need `gones.auth.sessionOwner` added to it. `npm run e2e:ci` and `npm run cy:run` were deliberately not run for this slice (plan Scope Out).

## POST-REVIEW registration-username-split

Landed as `55ae4f1`, after the plan's reviewer fanout. It repairs a dead end introduced by `be8239e`
(the generic-registration-answer slice): a fresh email combined with an already-taken username
returned `202 Accepted`, so the UI sent the person to `/verify-email` to wait for a mail that was
never queued, and no account was ever created. Retrying reproduced it forever.

The email-existence oracle that `be8239e` closed stays closed — a duplicate **email** still answers
the byte-identical generic `202`. Only a duplicate **username** now names itself, which discloses
nothing new because usernames are already public on participant lists.

- [ ] Register with a **fresh email** and a **username that already exists**. You must stay on the
      register page, see a message under the Username field in your current language (fr or en), and
      NOT be navigated to `/verify-email`. No email arrives.
- [ ] Register again with the **same fresh email** and a **free username**. It now succeeds normally
      and the verification mail arrives — proving the first attempt consumed nothing.
- [ ] Register with an **already-registered email** and a free username. You must get the generic
      "check your inbox" answer, land on `/verify-email`, and the existing account must receive a
      fresh verification link. The response must not mention the username.
- [ ] Register with an **already-registered email** and an **already-taken username**. Same generic
      answer as the previous step — the username must NOT be named. This is the case that would
      otherwise leak the email's existence through the username error.
- [ ] Switch the UI to French and repeat the first case; the field message must be French, not a raw
      key and not English.
