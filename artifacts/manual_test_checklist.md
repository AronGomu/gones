...
- [ ] **The four gates are green.** `npm run test` prints `Test Files 170 passed (170)` and `Tests 2106 passed (2106)` with no failed suite — T13's own run reported 166 / 2020, and T14 added the four archive suites on top; `npm run typecheck` exits `0` with no output; `npm run lint` prints `All files pass linting.`; `npm run build` exits `0`. T13's slice grew the count by exactly 122 over T12's 1898 — 8 shell tests and 114 list tests.
...
- [ ] **The four gates are green.** `npm run test` prints `Test Files 170 passed (170)` and `Tests 2106 passed (2106)`; `npm run typecheck` exits `0` with no output; `npm run lint` prints `All files pass linting.`; `npm run build` exits `0`. `npm run e2e:ci` prints `=== e2e specs: 27/27 passed ===` — no Cypress spec was edited by this slice.
